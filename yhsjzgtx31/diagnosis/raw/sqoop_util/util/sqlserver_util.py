...
class SqlServer():
    ...
    def __init__(self, section, filename):
        ...
        try:
            self._conn = SqlServer.__getConn(config, section)
    ...
    @staticmethod
    def __getConn(config, section):
        """
        @summary: 静态方法，从连接池中取出连接
        @return MySQLdb.connection
        """
        if section not in SqlServer.__pool:
            pool = PooledDB(creator=pymssql, mincached=1, maxcached=20,
                            host=config.get(section, 'host'),
                            port=config.getint(section, 'port'),
                            user=config.get(section, 'user'),
                            password=config.get(section, 'passwd'),
                            database=config.get(section, 'dbname'))
            SqlServer.__pool[section] = pool
        return SqlServer.__pool[section].connection()
    ...
    def _exeCute(self, sql=''):
        try:
            ...
            error = 'SqlServer execute failed! ERROR (%s): %s' % (e.args[0], e.args[1])
            print error
...
#     sqlserver = SqlServer('dorm_info','/Users/catherine/IdeaProjects/BigData.ETL.SqoopUtil/bs.sqlserver.conf')
#     sql = "SELECT Name FROM SysObjects Where XType='U' ORDER BY Name "
#
#     rows = sqlserver._exeCute(sql)
#     print rows;
