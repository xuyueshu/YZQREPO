...
cd `dirname $0`
source ./config.sh
exec_dir planning_completion_rate
...
function import_table() {
    find_mysql_data "
        ...
        from
           (
           select
           CONCAT(cast(count(case when a.plan_layer = 'G_TEACHER' and a.plan_status = 'YWC' then 1 end)
           / count(case when a.plan_layer = 'G_TEACHER' then 1 end)*100 as decimal(9,2)),'','%') as num,
           a.start_date as data_time
           from
           ...
           where b.data_no='GHWCL'
    "
    fn_log "导出——规划完成率：${TARGET_TABLE}"
