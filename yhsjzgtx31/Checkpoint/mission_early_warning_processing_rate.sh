...
cd `dirname $0`
source ./config.sh
exec_dir mission_early_warning_processing_rate
TARGET_TABLE=ef_assess_point_data_value_info
#DATA_NAME=任务预警处理率
DATA_NO=RWYJCLL
#count(case when c.warn_status='YGB' then  c.task_no end ) as num2
function import_table() {
    find_mysql_data "
        ...
        from
          (select
		   ...
		   sum(case when c.warn_status='YGB' then 1 else 0 end) as num2
           from
           tm_task_info a
		   left join
    ...
    "
       fn_log "创建表——任务预警处理率：${TARGET_TABLE}"
}
