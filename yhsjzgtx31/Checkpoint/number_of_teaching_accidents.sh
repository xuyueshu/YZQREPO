...
cd `dirname $0`
source ./config.sh
exec_dir number_of_teaching_accidents
...
#抽取SEMESTER_YEARS的数据，根据第一学期和第二学期进行数据抽取
