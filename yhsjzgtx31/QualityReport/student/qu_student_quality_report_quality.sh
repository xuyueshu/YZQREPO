...
cd `dirname $0`
source ../config.sh
exec_dir qu_student_quality_report_quality
#学生层面质控点信息
HIVE_DB=assurance
TARGET_TABLE=qu_student_quality_report_quality
function create_table() {
    ...
    hive -e "CREATE EXTERNAL TABLE IF NOT EXISTS ${HIVE_DB}.${HIVE_TABLE}(
                ...
                student_num String comment '质控点适用学生人数',
    ...
    fn_log "创建表——学生层面质控点信息：${HIVE_DB}.${HIVE_TABLE}"
}
...
find_mysql_data "
         ...
         report_no,index_no,index_name,quality_name,standard_rate,target_rate,student_num,create_time
         )
             select
                ...
                (
                    ...
                    count(ic.student_no) as student_num
                    from
                    im_quality_info im
                    left join
                    im_student_target_standard_record ic
                    on im.quality_no=ic.quality_no
                    where im.quality_status='NORMAL'
                    and im.index_layer='STUDENT'
                ...
                where a.index_layer='STUDENT' and a.index_level='SECOND'
            ...
            "
    fn_log "导入数据 —— 学生层面质控点信息 ${ITEM_KEY}：${HIVE_DB}.${HIVE_TABLE}"
