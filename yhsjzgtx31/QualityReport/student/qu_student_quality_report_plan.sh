...
cd `dirname $0`
source ../config.sh
exec_dir qu_student_quality_report_plan
#学院层面质控点信息

HIVE_DB=assurance
TARGET_TABLE=qu_student_quality_report_plan
...
find_mysql_data "
        set names utf8;
         ...
         plan_finish,task_finish,create_time)
            select
                concat('${SEMESTER_YEARS}','${SEMESTERS}','STUDENT')  as report_no,
                XYCMYWCGH.plan_name,
                XYCMYWCGH.plan_no,
				...
				and a.plan_layer='G_STUDENT' and c.task_delete='NORMAL'
                ...
                where a.relation_plan_no is null or a.relation_plan_no='' and a.status='NORMAL'
				and a.plan_layer='G_STUDENT' and c.task_delete='NORMAL' and a.start_date between '${BEGIN_TIME}' and '${END_TIME}'
				and c.start_date between '${BEGIN_TIME}' and '${END_TIME}'
           ...
           "
    fn_log "导入数据 —— 教师整体规划完成情况 ${ITEM_KEY}：${HIVE_DB}.${HIVE_TABLE}"
}
