...
cd `dirname $0`
source ../config.sh
exec_dir qu_course_quality_report_quality
#3课程层面质控点信息
HIVE_DB=assurance
TARGET_TABLE=qu_course_quality_report_quality
...
#teacher_num String comment '质控点适用课程数',
...
find_mysql_data "
         ...
         report_no,index_no,index_name,quality_name,standard_rate,target_rate,course_num,create_time
         )
             select
                ...
                (
                    ...
                    count(ic.course_code) as course_num
                    from
                    im_quality_info im
                    left join
                    im_course_target_standard_record ic
                    on im.quality_no=ic.quality_no
                    where im.quality_status='NORMAL'
                    and im.index_layer='COURSE'
                ...
                where a.index_layer='COURSE' and a.index_level='SECOND'
            ...
            "
    fn_log "导入数据 —— 课程层面质控点信息 ${ITEM_KEY}：${HIVE_DB}.${HIVE_TABLE}"
}
