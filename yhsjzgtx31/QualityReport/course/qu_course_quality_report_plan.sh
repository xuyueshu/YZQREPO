...
cd `dirname $0`
source ../config.sh
exec_dir qu_course_quality_report_plan
#教师整体规划完成情况

HIVE_DB=assurance
TARGET_TABLE=qu_course_quality_report_plan
...
find_mysql_data "
         ...
         plan_finish,task_finish,create_time)
             select
				...
				and a.plan_layer='G_COURSE' and c.task_delete='NORMAL'
                ...
                where a.relation_plan_no is null or a.relation_plan_no='' and a.status='NORMAL'
				and a.plan_layer='G_COURSE' and c.task_delete='NORMAL' and a.start_date between '${BEGIN_TIME}' and '${END_TIME}'
				and c.start_date between '${BEGIN_TIME}' and '${END_TIME}'
