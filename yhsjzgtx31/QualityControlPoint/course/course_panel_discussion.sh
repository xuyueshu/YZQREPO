...
cd `dirname $0`
source ./../config.sh
exec_dir course_panel_discussion

HIVE_DB=assurance
HIVE_TABLE=course_panel_discussion
...
function create_table() {
    ...
    fn_log "创建表——小组讨论：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            "
    fn_log "导入数据 —— 小组讨论：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
               ...
               b.name
                ...
                assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
                and a.semester_year in (select max(s.semester_year) from model.course_implement s)
         "
}
