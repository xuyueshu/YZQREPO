...
cd `dirname $0`
source ./../config.sh
exec_dir course_picture_quantity

HIVE_DB=assurance
HIVE_TABLE=course_picture_quantity
...
function create_table() {
    ...
    fn_log "创建表——图片数量：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                (select a.semester_year,a.semester,a.course_code,a.course_name,sum(a.picture_num) as c from model.course_resource a
            ...
            "
    fn_log "导入数据 —— 图片数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
