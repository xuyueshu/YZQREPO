...
cd `dirname $0`
source ./../config.sh
exec_dir course_pass_rate

HIVE_DB=assurance
HIVE_TABLE=course_pass_rate
...
function create_table() {
    ...
    fn_log "创建表——及格率：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            from
                (select count(0) as num,semester_year,course_code,course_name from model.student_score_record where level!=4 group by semester_year,course_code,course_name) a,
            ...
            "
    fn_log "导入数据 —— 及格率：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                ...
                assurance.im_quality_data_base_info c
                where a.semester_year=b.semester_year and a.course_code=b.course_code and c.data_name = '${DATA_NAME}'
                and a.semester_year in (select max(a.semester_year) from model.student_score_record a)
