...
cd `dirname $0`
source ./../config.sh
exec_dir course_operation_times

HIVE_DB=assurance
HIVE_TABLE=course_operation_times
...
function create_table() {
    ...
    fn_log "创建表——作业次数：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
       ...
       from
        (select a.semester_year,a.semester,a.course_code,b.name as course_name,
        sum(homework_num) as num from model.course_implement a
        left join model.basic_course_info b on a.course_code = b.code
        ...
        where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 作业次数：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table_new() {
    hive -e "
       ...
       from
        ...
        sum(homework_num) as num from model.course_implement a
        left join model.basic_course_info b on a.course_code = b.code
        where a.semester_year in (select max(a.semester_year) from model.course_implement a)
