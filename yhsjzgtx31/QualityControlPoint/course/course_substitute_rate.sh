...
cd `dirname $0`
source ./../config.sh
exec_dir course_substitute_rate

HIVE_DB=assurance
HIVE_TABLE=course_substitute_rate
...
function create_table() {
    ...
    fn_log "创建表——代课率：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
       ...
       where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 代课率：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
