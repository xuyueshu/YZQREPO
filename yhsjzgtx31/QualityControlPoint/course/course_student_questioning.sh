...
cd `dirname $0`
source ./../config.sh
exec_dir course_student_questioning

HIVE_DB=assurance
HIVE_TABLE=course_student_questioning
...
function create_table() {
    ...
    fn_log "创建表——学生提问率：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
               (select semester_year,semester,course_code,
               cast(avg(questions_rate) as decimal(9,2)) as num from model.course_implement
               group by semester_year,semester,course_code) a,model.basic_course_info b,
                assurance.im_quality_data_base_info c
                where a.course_code = b.code and c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学生提问率：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
