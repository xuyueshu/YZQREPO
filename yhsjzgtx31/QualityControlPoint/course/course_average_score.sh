...
cd `dirname $0`
source ./../config.sh
exec_dir course_average_score

HIVE_DB=assurance
HIVE_TABLE=course_average_score
...
function create_table() {
    ...
    fn_log "创建表——总评平均成绩：${HIVE_DB}.${HIVE_TABLE}"
}
          ...
          select
            ...
            from
             (
                      ...
                      cast(avg(a.avg_score) as decimal(9,2)) as num
                      from
            ...
            "
    fn_log "导入数据 —— 总评平均成绩：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
             (
                      ...
                      (select max(semester_year) from app.course_feedback)
