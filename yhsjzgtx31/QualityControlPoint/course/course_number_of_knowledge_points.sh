...
cd `dirname $0`
source ./../config.sh
exec_dir course_number_of_knowledge_points

HIVE_DB=assurance
HIVE_TABLE=course_number_of_knowledge_points
...
function create_table() {
    ...
    fn_log "创建表——知识点数量：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                (select a.semester_year,a.semester,a.course_code,a.course_name,sum(a.knowledge_points_num) as c from model.course_resource a
            ...
            "
    fn_log "导入数据 —— 知识点数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                      (
                      select a.semester_year,a.course_code,a.course_name,sum(a.knowledge_points_num) as c from model.course_resource a
                    where a.semester_year in (select max(semester_year) from model.course_resource)
                    group by a.semester_year,a.course_code,a.course_name
                      ) a,
                      assurance.im_quality_data_base_info c
                      where c.data_name = '${DATA_NAME}'
         "
