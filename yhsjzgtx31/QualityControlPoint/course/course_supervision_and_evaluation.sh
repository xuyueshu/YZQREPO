...
cd `dirname $0`
source ./../config.sh
exec_dir course_supervision_and_evaluation

HIVE_DB=assurance
HIVE_TABLE=course_supervision_and_evaluation
...
function create_table() {
    ...
    fn_log "创建表——督导评价分数：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            from
                (select
                 semester_year,course_code,cast(avg(evaluate_score) as decimal(9,2)) as num from app.course_feedback group by semester_year,course_code) a,model.basic_course_info b,
                assurance.im_quality_data_base_info c
                where a.course_code = b.code and c.data_no = '${DATA_NO}'
            "
    fn_log "导入数据 —— 督导评价分数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                ...
                assurance.im_quality_data_base_info c
                where a.course_code = b.code and c.data_no = '${DATA_NO}'
                and a.semester_year in (select max(s.semester_year) from app.course_feedback s )

         "
}
function export_table() {
    DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE}"`
    clear_mysql_data "delete from im_quality_course_data_info where data_no = '${DATA_NO}';"
    ...
    --columns 'data_no,data_name,course_name,course_code,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_course_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
     clear_mysql_data "update assurance.im_quality_course_data_info set is_new = 'YES' where data_no = '${DATA_NO}' and data_time='${DATE_TIME}';"
