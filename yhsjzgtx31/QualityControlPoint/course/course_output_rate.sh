...
cd `dirname $0`
source ./../config.sh
exec_dir course_output_rate

HIVE_DB=assurance
HIVE_TABLE=course_output_rate
...
function create_table() {
    ...
    fn_log "创建表——开出项目率：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
                ...
                cast(a.opencount/b.opencount*100 as decimal(9,2)) as data_value,
                'NO' as is_new,
            ...
            from
                ...
                count(a.training_name) as opencount
                from model.course_training_info a
                left join
                ...
                a.course_code,
                b.name)b
                on a.semester_year=b.semester_year and a.semester=b.semester
                and a.course_code=b.course_code,
                assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 开出项目率：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                ...
                a.semester_year in (select max(s.semester_year) from model.course_training_info s)
         "
}
