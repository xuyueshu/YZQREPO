...
cd `dirname $0`
source ./../config.sh
exec_dir course_Job_layout

HIVE_DB=assurance
HIVE_TABLE=course_Job_layout
...
function create_table() {
    ...
    fn_log "创建表——作业布置批改率：${HIVE_DB}.${HIVE_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
          select
               ...
               from
                      ...
                      avg(a.work_correction_rate) as num
                      from
                      model.course_implement a left join model.basic_course_info b
            ...
            "
    fn_log "导入数据 —— 作业布置批改率：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
               ...
               from
                      ...
                      model.course_implement a left join model.basic_course_info b
                      on a.course_code=b.code
                      where a.semester_year in (select max(semester_year) from model.course_implement)
