...
cd `dirname $0`
source ./../config.sh
exec_dir course_posting_of_forum_courses

HIVE_DB=assurance
HIVE_TABLE=course_posting_of_forum_courses
...
function create_table() {
    ...
    fn_log "创建表——论坛课程发帖率：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
          select
            ...
            from
               (select a.semester_year,a.semester,a.course_code,b.name as course_name,avg(forum_posting_rate) as num
            ...
            "
    fn_log "导入数据 —— 论坛课程发帖率：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
