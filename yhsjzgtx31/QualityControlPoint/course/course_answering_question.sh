...
cd `dirname $0`
source ./../config.sh
exec_dir course_answering_question
#答疑次数
HIVE_DB=assurance
HIVE_TABLE=course_answering_question
...
function create_table() {
    ...
    fn_log "创建表——答疑次数：${HIVE_DB}.${HIVE_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
          select
               ...
               from
                      ...
                      a.semester,
                      a.course_code,
                      sum(a.online_answer_num) as num
                      from
                      model.course_implement a
                      ...
                      and  a.course_code=b.code
            "
    fn_log "导入数据 —— 答疑次数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
               ...
               from
                      ...
                      where c.data_name = '${DATA_NAME}'
                      and  a.course_code=b.code
                      and a.semester_year in (select max(semester_year) from model.course_implement)
         "
}
