...
cd `dirname $0`
source ./../config.sh
exec_dir course_text_quantity

HIVE_DB=assurance
HIVE_TABLE=course_text_quantity
...
function create_table() {
    ...
    fn_log "创建表——文本数量：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                (select a.semester_year,a.semester,a.course_code,a.course_name,cast(sum(a.text_num) as int) as c from model.course_resource a
            ...
            "
    fn_log "导入数据 —— 文本数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                ...
                assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
                and a.semester_year in (select max(s.semester_year) from model.course_resource s )
         "
}
