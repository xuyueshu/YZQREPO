...
cd `dirname $0`
source ./../config.sh
exec_dir course_student_evaluation_of_teaching

HIVE_DB=assurance
HIVE_TABLE=course_student_evaluation_of_teaching
...
function create_table() {
    ...
    fn_log "创建表——学生评教分数：${HIVE_DB}.${HIVE_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            from
                ...
                semester_year,
                course_code,
                cast(avg(student_score) as decimal(9,2)) as num
                from app.course_feedback
                group by semester_year,course_code,student_score) a,model.basic_course_info b,
                assurance.im_quality_data_base_info c
                where a.course_code = b.code and c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学生评教分数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                ...
                assurance.im_quality_data_base_info c
                where a.course_code = b.code and c.data_name = '${DATA_NAME}'
                and a.semester_year in (select max(semester_year) from app.course_feedback)
         "
}
