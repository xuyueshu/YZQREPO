...
cd `dirname $0`
source ./../config.sh
exec_dir course_number_of_student_problem_sets

HIVE_DB=assurance
HIVE_TABLE=course_number_of_student_problem_sets
...
function create_table() {
    ...
    fn_log "创建表——学生习题集数量：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
          select
            ...
            from
                   (select a.semester_year,a.semester,a.course_code,a.course_name,sum(a.online_exercises_num) as c from model.course_resource a
                group by a.semester_year,a.semester,a.course_code,a.course_name) a,
                assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'

            "
    fn_log "导入数据 —— 学生习题集数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
