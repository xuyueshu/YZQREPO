...
cd `dirname $0`
source ./../config.sh
exec_dir course_training_project

HIVE_DB=assurance
HIVE_TABLE=course_training_project
...
function create_table() {
    ...
    fn_log "创建表——实训项目数：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            "
    fn_log "导入数据 —— 实训项目数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                ...
                model.basic_course_info b
                on a.course_code=b.code
                where a.semester_year in (select max(a.semester_year) from model.course_training_info a)
