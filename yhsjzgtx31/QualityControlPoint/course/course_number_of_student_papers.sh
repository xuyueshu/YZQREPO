...
cd `dirname $0`
source ./../config.sh
exec_dir course_number_of_student_papers

HIVE_DB=assurance
HIVE_TABLE=course_number_of_student_papers
...
function create_table() {
    ...
    fn_log "创建表——学生试卷数量：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                (select a.semester_year,a.semester,a.course_code,a.course_name,sum(a.online_examination_paper_num) as c from model.course_resource a
            ...
            "
    fn_log "导入数据 —— 学生试卷数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
