...
cd `dirname $0`
source ./../config.sh
exec_dir major_test_passing_rate

HIVE_DB=assurance
HIVE_TABLE=major_test_passing_rate
...
function create_table() {
    ...
    fn_log "创建表——考试通过率表：${HIVE_DB}.${HIVE_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                  c.data_no as data_no,
                   ...
                   concat(a.semester_year,'-',semester) as data_time,
                   a.num as data_value,
                  'NO' as is_new,
                 FROM_UNIXTIME(
                        UNIX_TIMESTAMP()
            ...
            from
               (
                    ...
                    a.major_code,
                    a.major_name,
                    concat( cast((count(case when b.score>'60' then b.code  end )
                     / count(a.code))*100 as decimal(9,2)),'','%') as num
                     from
                     app.basic_semester_student_info a left join
                     model.student_score_record b on a.code=b.code
            ...
            "
    fn_log "导入数据 —— 考试通过率表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--考试通过率表:${HIVE_DB}.${TARGET_TABLE}"
}
#导入新数据
            ...
            from
               (
                    ...
                    concat( cast((count(case when b.score>'60' then b.code  end )
                     ...
                     (select max(s.semester_year) from model.basic_semester_student_info s)
                     group by a.semester_year,
                     a.semester,
                     a.major_code,
                     a.major_name
