...
cd `dirname $0`
source ./../config.sh
exec_dir major_donation_device_price_count

HIVE_DB=assurance
HIVE_TABLE=major_donation_device_price_count
...
function create_table() {
    ...
    fn_log "创建表——捐赠设备总值表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
            model.major_donation_major_count a,
            assurance.im_quality_data_base_info c
            where c.data_no ='${DATA_NO}'


            "
    fn_log "导入数据 —— 捐赠设备总值表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
    ...
    fn_log "导出数据--捐赠设备总值表:${HIVE_DB}.${TARGET_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                (
                    select
                     ...
                     a.major_name,
                     a.semester_year,
                     a.donation_device_price_count
                     from
                     model.major_donation_major_count a
                     where a.semester_year in
                     (select max(semester_year) from model.major_donation_major_count)
