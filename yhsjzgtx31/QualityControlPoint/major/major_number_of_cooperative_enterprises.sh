...
cd `dirname $0`
source ./../config.sh
exec_dir major_number_of_cooperative_enterprises

HIVE_DB=assurance
HIVE_TABLE=major_number_of_cooperative_enterprises
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
             select
                   ...
                   a.cooperation_company_count as data_value,
                   'NO' as is_new,
            ...
            select
                   ...
                   FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                      (
                      select
                      t.cooperation_company_count,
                      t.major_code,
                      t.major_name,
                      t.semester_year
                      from app.major_donation_all_count a
                      where  t.semester_year in
                      (select max(s.semester_year) from app.major_donation_all_count s)
                      ) a,
...
function export_table() {
    ...
    fn_log "导出数据--合作企业数量:${HIVE_DB}.${TARGET_TABLE}"
}
