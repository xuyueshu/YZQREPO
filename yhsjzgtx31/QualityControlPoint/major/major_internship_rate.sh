...
cd `dirname $0`
source ./../config.sh
exec_dir major_internship_rate

HIVE_DB=assurance
HIVE_TABLE=major_internship_rate
...
function create_table() {
    ...
    fn_log "创建表——顶岗实习学生比例表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                ...
                concat(a.students_proportion,'','%') as data_value,
                'NO' as is_new,
                 FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
            ...
            "
    fn_log "导入数据 —— 顶岗实习学生比例表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
    ...
    fn_log "导出数据--顶岗实习学生比例表:${HIVE_DB}.${TARGET_TABLE}"
}
