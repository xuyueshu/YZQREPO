...
cd `dirname $0`
source ./../config.sh
exec_dir major_professional_proportion

HIVE_DB=assurance
HIVE_TABLE=major_professional_proportion
TARGET_TABLE=im_quality_major_data_info
DATA_NO=ZY_ZYSJXSZZYZXSBL
...
function create_table() {
    ...
    ) COMMENT '专业实践学时占专业总学时比例'
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——专业实践学时占专业总学时比例表：${HIVE_DB}.${HIVE_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                  c.data_no as data_no,
                   ...
                   CONCAT(cast((a.practice_hour / total_hour)*100 as decimal(9,2)),'','%') as data_value,
                    'NO' as is_new,
                 FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
            from
                 ...
                 a.is_open='1' and
                 a.semester_year in
                 (select max(s.semester_year) from  model.major_course_record s)


            "
    fn_log "导入数据 —— 专业实践学时占专业总学时比例表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--专业实践学时占专业总学时比例表:${HIVE_DB}.${TARGET_TABLE}"
}
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 app.major_course_record a,
                 assurance.im_quality_data_base_info c
                 where c.data_no ='${DATA_NO}' and
                 a.is_open='1' and
                 a.semester_year in
                 (select max(s.semester_year) from  app.major_course_record s)
