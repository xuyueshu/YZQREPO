...
cd `dirname $0`
source ./../config.sh
exec_dir major_employment_rate

HIVE_DB=assurance
HIVE_TABLE=major_employment_rate
...
function create_table() {
    ...
    fn_log "创建表——就业率表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                select
                   ...
                   a.employment_rate as data_value,
                   'NO' as is_new,
                   FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                   ) AS create_time
            ...
            "
    fn_log "导入数据 —— 就业率表：${HIVE_DB}.${HIVE_TABLE}"
}
