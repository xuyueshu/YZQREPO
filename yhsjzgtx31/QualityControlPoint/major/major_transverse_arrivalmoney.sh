...
cd `dirname $0`
source ./../config.sh
exec_dir major_transverse_arrivalmoney

HIVE_DB=assurance
HIVE_TABLE=major_transverse_arrivalmoney
TARGET_TABLE=im_quality_major_data_info
DATA_NO=ZY_HXKYXMDKE
...
function create_table() {
    ...
    fn_log "创建表——横向科研项目到款额：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                select
                   ...
                   a.transverse_amount as data_value,
                   'NO' as is_new,
            ...
            "
    fn_log "导入数据 —— 横向科研项目到款额：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--横向科研项目到款额:${HIVE_DB}.${TARGET_TABLE}"
}
#导入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 ...
                 (select max(s.semester_year) from app.major_scientific_info s )
