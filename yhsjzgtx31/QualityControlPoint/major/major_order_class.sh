...
cd `dirname $0`
source ./../config.sh
exec_dir major_order_class

HIVE_DB=assurance
HIVE_TABLE=major_order_class
...
function create_table() {
    ...
    fn_log "创建表——订单班培养人数：${HIVE_DB}.${HIVE_TABLE}"
}
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
             select
              ...
              from
                 (
                     ...
                     major_name,
                     semester_year
                     from app.major_plan_student
                     where type = '1' and major_code!=''
                     group by major_code,
                     major_name,
                     semester_year
                 ) a,
            ...
            "
    fn_log "导入数据 —— 订单班培养人数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
                   ...
                   'NO' as is_new,
                    ...
                    from
                      (
                          ...
                          t.semester_year
                          from
                          app.major_plan_student t
                          where type = '1 'and t.semester_year in
                          (select max(s.semester_year) from app.major_plan_student s)
                          group by major_code,
                          major_name,
                          semester_year
                      ) a,
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--订单班培养人数:${HIVE_DB}.${TARGET_TABLE}"
}
