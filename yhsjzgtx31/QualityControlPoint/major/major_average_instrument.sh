...
cd `dirname $0`
source ./../config.sh
exec_dir major_average_instrument

HIVE_DB=assurance
HIVE_TABLE=major_average_instrument
...
function create_table() {
    ...
    fn_log "创建表——生均仪器设备值：${HIVE_DB}.${HIVE_TABLE}"
}
function import_table() {
    ...
    a.price_avg as data_value,
    'NO' as is_new,
    FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
from model.major_trainingRoom_detailed  a,assurance.im_quality_data_base_info c
    where a.major_code!='' and c.data_no ='${DATA_NO}'
            "
    fn_log "导入数据 —— 生均仪器设备值：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
           ...
           select
              ...
              from
                    (
                        ...
                        a.major_name,
                        a.semester_year,
                        sum(a.price_avg) as num
                        from model.major_trainingRoom_detailed a
                        where a.is_school='1' and a.semester_year in
                        (select max(s.semester_year) from model.major_trainingRoom_detailed s)
                        group by a.major_code, a.major_name, a.semester_year
                    ) a,
...
function export_table() {
    ...
    fn_log "导出数据--生均仪器设备值:${HIVE_DB}.${TARGET_TABLE}"
}
