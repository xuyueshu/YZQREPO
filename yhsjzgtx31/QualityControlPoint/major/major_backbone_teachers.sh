...
cd `dirname $0`
source ./../config.sh
exec_dir major_backbone_teachers

HIVE_DB=assurance
HIVE_TABLE=major_backbone_teachers
...
function create_table() {
    ...
    fn_log "创建表——有无骨干教师：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                   ...
                   case when a.zbc >0 then '有' else '无' end as data_value,
                   'NO' as is_new,
                   FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                    (
                        select
                        count(case when s.is_core_teacher ='1' then s.is_core_teacher end) as zbc,
                        s.major_code,
                        s.major_name,
                        s.semester_year
                        from model.basic_teacher_info s
            ...
            "
    fn_log "导入数据 —— 有无骨干教师：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
           ...
           select
                   ...
                   FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    ...
                    (
                        ...
                        s.semester_year
                        from model.basic_teacher_info s
                        where s.is_quit='0' and
                        s.semester_year in
                        (select max(a.semester_year) from model.basic_teacher_info a)
                        group by s.major_code,s.major_name,s.semester_year
                    )a,
                     assurance.im_quality_data_base_info c
                    where data_no ='${DATA_NO}'
...
function export_table() {
    ...
    fn_log "导出数据--有无骨干教师:${HIVE_DB}.${TARGET_TABLE}"
}
