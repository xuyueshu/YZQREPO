...
cd `dirname $0`
source ./../config.sh
exec_dir major_teacher_abroad

HIVE_DB=assurance
HIVE_TABLE=major_teacher_abroad
...
function create_table() {
    ...
    fn_log "创建表——教师境外交流（人日）表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
                 ...
                 a.teacher_man_day as data_value,
                 'NO' as is_new,
                 FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
            ...
            "
    fn_log "导入数据 —— 教师境外交流（人日）表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--教师境外交流（人日）表:${HIVE_DB}.${TARGET_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            from
              ...
              (select s.semester_year from app.major_abroad_communication_count s)
