...
cd `dirname $0`
source ./../config.sh
exec_dir major_transverse_scientific

HIVE_DB=assurance
HIVE_TABLE=major_transverse_scientific
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
              select
                   ...
                   a.transverse_project_num as data_value,
                   'NO' as is_new,
            ...
            "
    fn_log "导入数据 —— 横向科研项目数量：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--横向科研项目数量:${HIVE_DB}.${TARGET_TABLE}"
}
