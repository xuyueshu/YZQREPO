...
cd `dirname $0`
source ./../config.sh
exec_dir major_teaching_resource_bank

HIVE_DB=assurance
HIVE_TABLE=major_teaching_resource_bank
TARGET_TABLE=im_quality_major_data_info
DATA_NAME=专业教学资源库数量
DATA_NO=ZY_ZYJXZYKSL
function create_table() {
    ...
    fn_log "创建表——专业教学资源库数量：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                   ...
                   a.Resources as data_value,
                    'NO' as is_new,
                 FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
             (
                select
                   sum(national_resourses_count) +
                    sum(provincial_resources_count) +
                    sum(college_resources_count) as Resources,
                    major_code,
                    major_name,
                    semester_year
                    from
                    model.major_instructional_resources
                    group by major_code,
                    major_name,
                    semester_year
              ) a,
            ...
            "
    fn_log "导入数据 —— 专业教学资源库数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
             ...
             (
                select
                   sum(national_resourses_count) +
                    ...
                    from
                    model.major_instructional_resources
                    where semester_year in
                    (select max(s.semester_year) from model.major_instructional_resources s)
                    group by major_code,
                    major_name,
                    semester_year
              ) a,
...
DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
    ...
    fn_log "导出数据--专业教学资源库数量:${HIVE_DB}.${TARGET_TABLE}"
}
#导出新数据
