...
cd `dirname $0`
source ./../config.sh
exec_dir major_internship_accept_rate

HIVE_DB=assurance
HIVE_TABLE=major_internship_accept_rate
TARGET_TABLE=im_quality_major_data_info
DATA_NO=ZY_DGSXLYXSBL
...
function create_table() {
    ...
    fn_log "创建表——顶岗实习录用学生比例表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                   ...
                   concat(a.enroll_students_proportion,'','%') as data_value,
                   'NO' as is_new,
                    FROM_UNIXTIME(
                        UNIX_TIMESTAMP()
                      ) AS create_time
            from
               model.major_post_practice_count a  ,assurance.im_quality_data_base_info c
               where c.data_no ='${DATA_NO}'

            "
    fn_log "导入数据 —— 顶岗实习录用学生比例表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
    ...
    fn_log "导出数据--顶岗实习录用学生比例表:${HIVE_DB}.${TARGET_TABLE}"
}
