...
cd `dirname $0`
source ./../config.sh
exec_dir major_portrait_arrivalmoney

HIVE_DB=assurance
HIVE_TABLE=major_portrait_arrivalmoney
TARGET_TABLE=im_quality_major_data_info
DATA_NO=ZY_ZXKYXMDKE
...
function create_table() {
    ...
    fn_log "创建表——纵向科研项目到款额表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                select
                   ...
                   a.portrait_amount as data_value,
                   'NO' as is_new,
            ...
            "
    fn_log "导入数据 —— 纵向科研项目到款额表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--纵向科研项目到款额表:${HIVE_DB}.${TARGET_TABLE}"
}
#导入新数据
