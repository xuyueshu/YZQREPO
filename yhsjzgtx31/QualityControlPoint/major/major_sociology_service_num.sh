...
cd `dirname $0`
source ./../config.sh
exec_dir major_sociology_service_num

HIVE_DB=assurance
HIVE_TABLE=major_sociology_service_num
...
function create_table() {
    ...
    fn_log "创建表——社会服务项目数表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            "
    fn_log "导入数据 —— 社会服务项目数表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--社会服务项目数表:${HIVE_DB}.${TARGET_TABLE}"
}
