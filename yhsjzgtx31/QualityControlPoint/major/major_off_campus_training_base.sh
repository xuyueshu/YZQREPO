...
cd `dirname $0`
source ./../config.sh
exec_dir major_off_campus_training_base

HIVE_DB=assurance
HIVE_TABLE=major_off_campus_training_base
...
function create_table() {
    ...
    fn_log "创建表——校外实训基地数量：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
             select
                   ...
                   a.trainingRoom as data_value,
                   'NO' as is_new,
                   FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                   (
                       select count(training_base_id) as trainingRoom,
                       major_code,
                       major_name,
                       semester_year
                       from
                       model.major_trainingRoom_detailed where is_school = '2' and major_code!=''
                       group by major_code,
                       major_name,
                       semester_year
                   ) a,
            ...
            "
    fn_log "导入数据 —— 校外实训基地数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
            ...
            select
                   ...
                   'NO' as is_new,
                    ...
                    from
                      (
                      select
                      count(t.training_base_id) as trainingRoom,
                      t.major_code,
                      t.major_name,
                      t.semester_year
                      from model.major_trainingRoom_detailed t
                      where is_school = '2' and t.semester_year in
                      (select max(semester_year) from model.major_trainingRoom_detailed)
                      group by t.major_code,t.major_name,t.semester_year
                      ) a,
...
function export_table() {
    ...
    fn_log "导出数据--校外实训基地数量:${HIVE_DB}.${TARGET_TABLE}"
}
