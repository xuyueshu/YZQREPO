...
cd `dirname $0`
source ./../config.sh
exec_dir major_teachering_hours

HIVE_DB=assurance
HIVE_TABLE=major_teachering_hours
...
function create_table() {
    ...
    fn_log "创建表——专业教学总课时量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 专业教学总课时量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--专业教学总课时量表:${HIVE_DB}.${TARGET_TABLE}"
}
            ...
            from
                 (
                    select
                      ...
                      (select max(s.semester_year) from app.basic_teacher_info)
                      group by a.semester,
                      a.major_code,
                      a.major_name
                     ) a,assurance.im_quality_data_base_info c
