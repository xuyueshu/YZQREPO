...
cd `dirname $0`
source ./../config.sh
exec_dir major_innovate_award_num

HIVE_DB=assurance
HIVE_TABLE=major_innovate_award_num
TARGET_TABLE=im_quality_major_data_info
DATA_NO=ZY_XSCXCYHJSL
...
function create_table() {
    ...
    fn_log "创建表——学生创新创业获奖数量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 学生创新创业获奖数量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
    ...
    fn_log "导出数据--学生创新创业获奖数量表:${HIVE_DB}.${TARGET_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
              (
                ...
                app.basic_semester_student_info a
                left join app.student_award_record b on a.code=b.code
                where b.award_type = 'CXCY' and  a.semester_year in
