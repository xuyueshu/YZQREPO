...
cd `dirname $0`
source ./../config.sh
exec_dir major_registration_rate

HIVE_DB=assurance
HIVE_TABLE=major_registration_rate
...
function create_table() {
    ...
    fn_log "创建表——报到率：${HIVE_DB}.${HIVE_TABLE}"
}
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 报到率：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
...
function export_table() {
    ...
    fn_log "导出数据--报到率:${HIVE_DB}.${TARGET_TABLE}"
}
#导出新数据
