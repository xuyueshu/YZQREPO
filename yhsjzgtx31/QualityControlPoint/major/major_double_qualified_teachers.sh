...
cd `dirname $0`
source ./../config.sh
exec_dir major_double_qualified_teachers

HIVE_DB=assurance
HIVE_TABLE=major_double_qualified_teachers
...
function create_table() {
    ...
    fn_log "创建表——双师素质教师比例：${HIVE_DB}.${HIVE_TABLE}"
}
...
from
    ...
    major_name,
    semester_year,
    cast(count(case when is_double_professionally='1' then code end)/count(0)*100 as decimal(9,2)) as num
    from
    ...
    where c.data_no ='${DATA_NO}'
            "
    fn_log "导入数据 —— 双师素质教师比例：${HIVE_DB}.${HIVE_TABLE}"
}
           ...
           select
            ...
            from
               (
                   ...
                   a.major_name,
                   a.semester_year
                   cast((count(case when a.is_double_professionally='1' then a.code end) /
                   count(a.code))*100 as decimal(9,2))) as num
                   from
                   model.basic_teacher_info a
                   where a.is_quit ='0'
                   and a.semester_year in (select max(s.semester_year) from model.basic_teacher_info s )
...
function export_table() {
    ...
    fn_log "导出数据--双师素质教师比例:${HIVE_DB}.${TARGET_TABLE}"
}
