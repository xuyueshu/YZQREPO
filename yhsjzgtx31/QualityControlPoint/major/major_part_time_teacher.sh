...
cd `dirname $0`
source ./../config.sh
exec_dir major_part_time_teacher

HIVE_DB=assurance
HIVE_TABLE=major_part_time_teacher
...
function create_table() {
    ...
    fn_log "创建表——有无兼职教师：${HIVE_DB}.${HIVE_TABLE}"
}
#ok
...
from
    ...
    major_name,
    semester_year,
    count( case when teacher_type like '%兼职%' then code end) as num
    from
    ...
    where c.data_no ='${DATA_NO}'
            "
    fn_log "导入数据 —— 有无兼职教师：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
           ...
           select
            ...
            from
                (
                    ...
                    count( case when teacher_type like '%兼职%' then code end) as num
                    from
                    model.basic_teacher_info where is_quit='0'
                    group by major_code,major_name,semester_year
                 ) a ,
                assurance.im_quality_data_base_info c
                where c.data_no ='${DATA_NO}'
...
function export_table() {
  DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
    ...
    fn_log "导出数据--有无兼职教师:${HIVE_DB}.${TARGET_TABLE}"
}
#导出新数据
function export_table_new() {
    ...
    --columns 'data_no,data_name,major_no,major_name,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_name = '${DATA_NAME}';"
     clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'YES' where data_name = '${DATA_NAME}' and data_time= '${DATE_TIME}' "
    fn_log "导出数据--就业率表:${HIVE_DB}.${TARGET_TABLE}"
}
