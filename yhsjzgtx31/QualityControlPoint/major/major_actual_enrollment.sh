...
cd `dirname $0`
source ./../config.sh
exec_dir major_actual_enrollment

HIVE_DB=assurance
HIVE_TABLE=major_actual_enrollment
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
                 ...
                 a.actual_enroll_student_count as data_value,
                 'NO' as is_new,
                 FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
            model.major_enroll_student a
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            (select max(s.semester_year) from model.major_enroll_student s )
         "
         fn_log "导入数据 —— 实际录取人数：${HIVE_DB}.${HIVE_TABLE}"
}
function export_table() {
    ...
    fn_log "导出数据--实际录取人数:${HIVE_DB}.${TARGET_TABLE}"
}
