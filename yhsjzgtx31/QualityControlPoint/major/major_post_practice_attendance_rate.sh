...
cd `dirname $0`
source ./../config.sh
exec_dir major_post_practice_attendance_rate

HIVE_DB=assurance
HIVE_TABLE=major_post_practice_attendance_rate
...
function create_table() {
    ...
    fn_log "创建表——顶岗实习签到率表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
             select
                   ...
                   concat(a.sign_in_rate_avg,'','%') as data_value,
                   'NO' as is_new,
                   FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                   ) AS create_time
            ...
            "
    fn_log "导入数据 —— 顶岗实习签到率表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--顶岗实习签到率表:${HIVE_DB}.${TARGET_TABLE}"
}
