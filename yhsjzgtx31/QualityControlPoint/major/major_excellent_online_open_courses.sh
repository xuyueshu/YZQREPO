...
cd `dirname $0`
source ./../config.sh
exec_dir major_excellent_online_open_courses

HIVE_DB=assurance
HIVE_TABLE=major_excellent_online_open_courses
TARGET_TABLE=im_quality_major_data_info
DATA_NO=ZY_JPZXKFKCSL
...
function create_table() {
    ...
    fn_log "创建表——精品在线开放课程数量：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                    ...
                    a.course_count as data_value,
                    'NO' as is_new,
                 FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                   app.major_goods_online_course a,
                   assurance.im_quality_data_base_info c where c.data_no ='${DATA_NO}'
            "
    fn_log "导入数据 —— 精品在线开放课程数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                  ...
                  FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                   ...
                   app.major_goods_online_course a,
                   assurance.im_quality_data_base_info c where c.data_no ='${DATA_NO}'
                   and major_code is not null and major_code <> ''
                   and a.semester_year in
                   (select max(s.semester_year) from app.major_goods_online_course s)
...
function export_table() {
    ...
    fn_log "导出数据--精品在线开放课程数量:${HIVE_DB}.${TARGET_TABLE}"
}
