...
cd `dirname $0`
source ./../config.sh
exec_dir major_training_room

HIVE_DB=assurance
HIVE_TABLE=major_training_room
...
function create_table() {
    ...
    fn_log "创建表——实训室使用率：${HIVE_DB}.${HIVE_TABLE}"
}
...
from
    ...
    major_code,
    major_name,
    cast(sum(actual_use_hours)/sum(plan_use_hours)*100 as decimal(9,2)) as num
    from
    model.major_trainingRoom_detailed
    where major_code !=''
    group by semester_year,major_code,major_name
    ) a,assurance.im_quality_data_base_info c
    where c.data_no ='${DATA_NO}'
            "
    fn_log "导入数据 —— 实训室使用率：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
           ...
           select
            ...
            (
                select
                    ...
                    a.major_code,
                    a.major_name,
                    cast(sum(a.actual_use_hours)/sum(a.plan_use_hours)*100 as decimal(9,2)) as num
                    from
                    model.major_trainingRoom_detailed a
...
function export_table() {
    ...
    fn_log "导出数据--实训室使用率:${HIVE_DB}.${TARGET_TABLE}"
}
