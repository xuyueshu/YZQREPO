...
cd `dirname $0`
source ./../config.sh
exec_dir major_international_communication

HIVE_DB=assurance
HIVE_TABLE=major_international_communication
...
function create_table() {
    ...
    fn_log "创建表——国际交流项目数表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                 ...
                 a.project_num as data_value,
                 'NO' as is_new,
                 FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
            from
              app.major_abroad_communication_count a ,
              assurance.im_quality_data_base_info c
              where  c.data_no ='${DATA_NO}'

            "
    fn_log "导入数据 —— 国际交流项目数表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
    ...
    fn_log "导出数据--国际交流项目数表:${HIVE_DB}.${TARGET_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
              app.major_abroad_communication_count a ,
              assurance.im_quality_data_base_info c
              where  c.data_no ='${DATA_NO}' and  a.semester_year in
              (select max(s.semester_year) from app.major_abroad_communication_count s)
