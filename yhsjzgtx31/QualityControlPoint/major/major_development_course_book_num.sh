...
cd `dirname $0`
source ./../config.sh
exec_dir major_development_course_book_num

HIVE_DB=assurance
HIVE_TABLE=major_development_course_book_num
...
function create_table() {
    ...
    fn_log "创建表——联合开发教材数量表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                   ...
                   a.material_count as data_value,
                   'NO' as is_new,
                   FROM_UNIXTIME(
                        UNIX_TIMESTAMP()
                      ) AS create_time
            ...
            "
    fn_log "导入数据 —— 联合开发教材数量表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 ...
                 a.semester_year in (select max(s.semester_year) from app.major_development_course s)
