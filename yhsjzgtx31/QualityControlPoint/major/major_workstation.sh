...
cd `dirname $0`
source ./../config.sh
exec_dir major_workstation

HIVE_DB=assurance
HIVE_TABLE=major_workstation
...
function create_table() {
    ...
    fn_log "创建表——工位数：${HIVE_DB}.${HIVE_TABLE}"
}
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 工位数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
...
DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
    ...
    fn_log "导出数据--工位数:${HIVE_DB}.${TARGET_TABLE}"
}
