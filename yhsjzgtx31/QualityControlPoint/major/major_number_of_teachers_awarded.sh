...
cd `dirname $0`
source ./../config.sh
exec_dir major_number_of_teachers_awarded

HIVE_DB=assurance
HIVE_TABLE=major_number_of_teachers_awarded
...
function create_table() {
    ...
    fn_log "创建表——教师获奖人数：${HIVE_DB}.${HIVE_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                c.data_no as data_no,
                   ...
                   a.teacher_number as data_value,
                    'NO' as is_new,
                 FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                (
                     select
                     count(1) as teacher_number,
                     b.major_code,
                     b.major_name,
                     a.semester_year
                     from model.teacher_awards_info a left join
                     model.basic_teacher_info b where a.code = b.code
                     and is_quit='2' and b.major_code is not null
                     group by
                     b.major_code,b.major_name,a.semester_year
                ) a,assurance.im_quality_data_base_info c where c.data_no ='${DATA_NO}'
            "
    fn_log "导入数据 —— 教师获奖人数：${HIVE_DB}.${HIVE_TABLE}"
}
                   ...
                   FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                      (
                         ...
                         from model.teacher_awards_info a left join
                         model.basic_teacher_info b where a.code = b.code
                         where a.is_quit='0' and a.semester_year in
                         (select max(s.semester_year) from model.teacher_awards_info s )
                         group by
                         b.major_code,b.major_name,a.semester_year
                      ) a,
                      assurance.im_quality_data_base_info c
                      where c.data_name = '${DATA_NAME}' and major_code is not null and major_code != ''
         "
           fn_log "导入数据 —— 教师获奖人数：${HIVE_DB}.${HIVE_TABLE}"
}
...
DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
    ...
    fn_log "导出数据--教师获奖人数:${HIVE_DB}.${TARGET_TABLE}"
}
