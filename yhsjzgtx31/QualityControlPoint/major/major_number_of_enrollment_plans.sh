...
cd `dirname $0`
source ./../config.sh
exec_dir major_number_of_enrollment_plans

HIVE_DB=assurance
HIVE_TABLE=major_number_of_enrollment_plans
...
function create_table() {
    ...
    fn_log "创建表——招生计划人数：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                   ...
                   a.plan_enroll_student_count as data_value,
                 'NO' as is_new,
            ...
            "
    fn_log "导入数据 —— 招生计划人数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                   ...
                   'NO' as is_new,
                    FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                     app.major_enroll_student a,
                     assurance.im_quality_data_base_info c
                     where c.data_name ='${DATA_NAME}'
                     and  a.semester_year in
                     (select max(s.semester_year) from app.major_enroll_student s )
...
function export_table() {
    ...
    fn_log "导出数据--招生计划人数:${HIVE_DB}.${TARGET_TABLE}"
}
