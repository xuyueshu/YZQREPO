...
cd `dirname $0`
source ./../config.sh
exec_dir major_fulltime_teacher

HIVE_DB=assurance
HIVE_TABLE=major_fulltime_teacher
...
function create_table() {
    ...
    fn_log "创建表——专任教师人数：${HIVE_DB}.${HIVE_TABLE}"
}
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            (
                SELECT
                count(a.code) as teacher_number,
                major_code,
                major_name,
                semester_year
                from model.basic_teacher_info a
                where a.teacher_type = '校内专任教师' and a.is_quit='0'
                GROUP BY major_code,major_name,semester_year) a,assurance.im_quality_data_base_info c
                where c.data_no ='${DATA_NO}'
            "
    fn_log "导入数据 —— 专任教师人数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            (
                ...
                GROUP BY major_code,major_name,semester_year) a,assurance.im_quality_data_base_info c
                where c.data_no ='${DATA_NO}'
                and a.semester_year in (select max(semester_year) from model.basic_teacher_info)
         "
          fn_log "导入数据 —— 专任教师人数：${HIVE_DB}.${HIVE_TABLE}"
}
function export_table() {
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
    ...
    fn_log "导出数据--专任教师人数:${HIVE_DB}.${TARGET_TABLE}"
}
