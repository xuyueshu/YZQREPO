...
cd `dirname $0`
source ./../config.sh
exec_dir major_development_course_num

HIVE_DB=assurance
HIVE_TABLE=major_development_course_num
...
function create_table() {
    ...
    fn_log "创建表——联合开发课程数量表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            from
                 app.major_development_course a ,
                 assurance.im_quality_data_base_info c
                 where c.data_no ='${DATA_NO}'

            "
    fn_log "导入数据 —— 联合开发课程数量表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            from
                 model.major_development_course a ,
                 assurance.im_quality_data_base_info c
                 where c.data_no ='${DATA_NO}'
                 and a.semester_year in
                 (select max(s.semester_year) from model.major_development_course s)
