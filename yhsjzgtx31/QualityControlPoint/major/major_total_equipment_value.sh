...
cd `dirname $0`
source ./../config.sh
exec_dir major_total_equipment_value

HIVE_DB=assurance
HIVE_TABLE=major_total_equipment_value
...
function create_table() {
    ...
    fn_log "创建表——设备总值：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                   ...
                   a.price_sum as data_value,
                   'NO' as is_new,
                   FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                    model.major_trainingRoom_detailed a,
                    assurance.im_quality_data_base_info c
                    where a.major_code!='' and c.data_name ='${DATA_NAME}'

            "
    fn_log "导入数据 —— 设备总值：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
...
DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
    ...
    fn_log "导出数据--设备总值:${HIVE_DB}.${TARGET_TABLE}"
}
#导出新数据
