...
cd `dirname $0`
source ./../config.sh
exec_dir major_graduation_rate

HIVE_DB=assurance
HIVE_TABLE=major_graduation_rate
...
function create_table() {
    ...
    fn_log "创建表——毕业率表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
                   ...
                   a.graduation_rate as data_value,
                   'NO' as is_new,
                   FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                   ) AS create_time
                ...
                (
                    select
                     ...
                     a.graduation_rate
                     from
                     app.student_graduate_employment_count a
                     left join
            ...
            "
    fn_log "导入数据 —— 毕业率表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
    ...
    fn_log "导出数据--毕业率表:${HIVE_DB}.${TARGET_TABLE}"
}
           ...
           select
                ...
                (
                    select
                     ...
                     (select max(s.semester_year) from app.student_graduate_employment_count s )
                 )a,
                assurance.im_quality_data_base_info c
                where c.data_no ='${DATA_NO}' and a.semester_year != ''
            "
