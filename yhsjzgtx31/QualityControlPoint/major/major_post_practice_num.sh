...
cd `dirname $0`
source ./../config.sh
exec_dir major_post_practice_num

HIVE_DB=assurance
HIVE_TABLE=major_post_practice_num
TARGET_TABLE=im_quality_major_data_info
DATA_NO=ZY_HZQYJSDGSXXSRS
...
function create_table() {
    ...
    ) COMMENT '合作企业接收顶岗实习学生人数表'
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"
    fn_log "创建表——合作企业接收顶岗实习学生人数表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
                   ...
                   a.enroll_students_num as data_value,
                   'NO' as is_new,
                   FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
            ...
            "
    fn_log "导入数据 —— 合作企业接收顶岗实习学生人数表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--合作企业接收顶岗实习学生人数表:${HIVE_DB}.${TARGET_TABLE}"
}
#导入新数据
