...
cd `dirname $0`
source ./../config.sh
exec_dir major_sociology_service_arrivalmoney

HIVE_DB=assurance
HIVE_TABLE=major_sociology_service_arrivalmoney
...
function create_table() {
    ...
    fn_log "创建表——社会服务到款额表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            from
                model.major_social_work a left join
                model.basic_major_info on a.major_code=b.code,
                assurance.im_quality_data_base_info c
                where  c.data_no ='${DATA_NO}'
            "
    fn_log "导入数据 —— 社会服务到款额表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
 DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
   ...
   clear_mysql_data "update assurance.im_quality_major_data_info set is_new = 'NO' where data_no ='${DATA_NO}';"
    ...
    fn_log "导出数据--社会服务到款额表:${HIVE_DB}.${TARGET_TABLE}"
}
            ...
            from
                (
                  select
                    ...
                    (select max(s.semester_year) from model.major_social_work s)
                ) a,
                 assurance.im_quality_data_base_info c
                 where  c.data_no ='${DATA_NO}'
            "
