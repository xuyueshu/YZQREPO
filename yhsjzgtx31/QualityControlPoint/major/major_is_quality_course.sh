...
cd `dirname $0`
source ./../config.sh
exec_dir major_is_quality_course

HIVE_DB=assurance
HIVE_TABLE=major_is_quality_course
TARGET_TABLE=im_quality_major_data_info
DATA_NO=ZY_SFKSSZTZLKC
...
function create_table() {
    ...
    fn_log "创建表——是否开设素质拓展类课程表：${HIVE_DB}.${HIVE_TABLE}"
}
           ...
           select
            ...
            from
               (
                ...
                a.major_code,
                a.major_name,
                count(case when a.developmentType='SZKZ' then a.course_code end) as num
                from
            ...
            "
    fn_log "导入数据 —— 是否开设素质拓展类课程表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
    ...
    fn_log "导出数据--是否开设素质拓展类课程表:${HIVE_DB}.${TARGET_TABLE}"
}
#导入新数据
