...
cd `dirname $0`
source ./../config.sh
exec_dir major_number_of_professional_teachers

HIVE_DB=assurance
HIVE_TABLE=major_number_of_professional_teachers
...
function create_table() {
    ...
    fn_log "创建表——专业教师数量：${HIVE_DB}.${HIVE_TABLE}"
}
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 专业教师数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
                    ...
                    from
                      (
                        SELECT
                         ...
                         where a.is_quit='0' and a.semester_year
                         in (select max(s.semester_year) from model.basic_teacher_info s)
...
function export_table() {
    ...
    fn_log "导出数据--专业教师数量:${HIVE_DB}.${TARGET_TABLE}"
}
