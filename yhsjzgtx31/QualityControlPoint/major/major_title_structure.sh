...
cd `dirname $0`
source ./../config.sh
exec_dir major_title_structure

HIVE_DB=assurance
HIVE_TABLE=major_title_structure
...
function create_table() {
    ...
    fn_log "创建表——职称结构：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            ( select
                ...
                a.major_code,
                a.major_name,
                concat_ws('、',collect_list(cast(a.professional_title as string))) as num
            ...
            "
    fn_log "导入数据 —— 职称结构：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
...
function export_table() {
  DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
    ...
    fn_log "导出数据--职称结构:${HIVE_DB}.${TARGET_TABLE}"
}
#导出新数据
