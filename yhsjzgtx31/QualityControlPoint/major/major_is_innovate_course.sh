...
cd `dirname $0`
source ./../config.sh
exec_dir major_is_innovate_course

HIVE_DB=assurance
HIVE_TABLE=major_is_innovate_course
TARGET_TABLE=im_quality_major_data_info
DATA_NO=ZY_SFKSCXCYKC
function create_table() {
    ...
    fn_log "创建表——是否开设创新创业类课程表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
               ...
               case when a.num > 0 then '是' else '否' end as data_value,
               'NO' as is_new,
                FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
            from
               (
                ...
                a.major_code,
                a.major_name,
                count(case when a.developmentType='CXCY' then a.course_code end) as num
                from
            ...
            "
    fn_log "导入数据 —— 是否开设创新创业类课程表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
    ...
    fn_log "导出数据--是否开设创新创业类课程表:${HIVE_DB}.${TARGET_TABLE}"
}
