...
cd `dirname $0`
source ./../config.sh
exec_dir teacher_participation_teaching_research_num

HIVE_DB=assurance
HIVE_TABLE=teacher_participation_teaching_research_num
...
function create_table() {
    ...
    fn_log "创建表——参与教研项目数量：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 (
                     select
                        ...
                        count(a.project_code) as num
                        from
                        model.teacher_teaching_research_personnel_info a left join
                        model.teacher_teaching_research_info b on a.project_code=b.project_code
                        where a.participate_type = '2'
            ...
            "
    fn_log "导入数据 —— 参与教研项目数量：${HIVE_DB}.${HIVE_TABLE}"
}
            ...
            from
                 (
                     select
                        ...
                        count(a.project_code) as num
                        from
                        model.teacher_teaching_research_personnel_info a
                        left join
                        model.teacher_teaching_research_info b
                        on a.project_code=b.project_code
                        where a.participate_type = '2' and b.semester_year in
                        (select max(s.semester_year) from model.teacher_teaching_research_info s)
                        group by b.semester_year,
                        a.code,
                        a.name
                  ) a,
...
function export_table_new() {
    ...
    --columns 'data_no,data_name,teacher_name,teacher_no,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_teacher_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
     clear_mysql_data "update assurance.im_quality_teacher_data_info set is_new = 'YES' where data_no = '${DATA_NO}' and data_time= '${DATE_TIME}' "
    fn_log "导出数据-- 参与教研项目数量表:${HIVE_DB}.${TARGET_TABLE}"
}
