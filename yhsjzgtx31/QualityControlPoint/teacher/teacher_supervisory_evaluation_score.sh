...
cd `dirname $0`
source ./../config.sh
exec_dir teacher_supervisory_evaluation_score

HIVE_DB=assurance
HIVE_TABLE=teacher_supervisory_evaluation_score
...
function create_table() {
    ...
    hive -e "CREATE EXTERNAL TABLE IF NOT EXISTS ${HIVE_DB}.${HIVE_TABLE}(
                data_no  String comment '数据项编号',
                 ...
                 teacher_name  String comment '教师编号',
                 teacher_no String comment '教师姓名',
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
                   ...
                   a.evaluate_score  as data_value,
                   'NO' as is_new,
                   FROM_UNIXTIME(
                        UNIX_TIMESTAMP()
                      ) AS create_time
            from
                 app.course_feedback a ,
                 assurance.im_quality_data_base_info c
                 where c.data_no ='${DATA_NO}' and a.semester_year != ''
            "
    fn_log "导入数据 —— 督导评价分数表：${HIVE_DB}.${HIVE_TABLE}"
}

function export_table() {
    clear_mysql_data "delete from im_quality_teacher_data_info where data_no = '${DATA_NO}';"
    ...
    fn_log "导出数据--督导评价分数表:${HIVE_DB}.${TARGET_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 app.course_feedback a ,
                 assurance.im_quality_data_base_info c
                 where c.data_no ='${DATA_NO}' and a.semester_year != ''  and
                 a.semester_year in (select max(s.semester_year) from app.course_feedback s)
