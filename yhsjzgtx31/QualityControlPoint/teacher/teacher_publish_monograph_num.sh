...
cd `dirname $0`
source ./../config.sh
exec_dir teacher_publish_monograph_num

HIVE_DB=assurance
HIVE_TABLE=teacher_publish_monograph_num
...
function create_table() {
    ...
    fn_log "创建表——出版专著数量：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 出版专著数量表：${HIVE_DB}.${HIVE_TABLE}"
}
           ...
           select
            ...
            from
                   ( select
                        ...
                        (select max(s.semester_year) from model.scientific_work_personnel_info s)
                        group by  a.semester_year,
                        a.teacher_code,
                        a.teacher_name
                    ) a,
