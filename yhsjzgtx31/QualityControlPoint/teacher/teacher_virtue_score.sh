...
cd `dirname $0`
source ././../config.sh
exec_dir teacher_virtue_score

HIVE_DB=assurance
HIVE_TABLE=teacher_virtue_score
...
function create_table() {
    ...
    fn_log "创建表——师德考核得分表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                   ...
                   a.character_score as data_value,
                   'NO' as is_new,
                FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
    ...
    "
    fn_log "导入数据 —— 师德考核得分表：${HIVE_DB}.${HIVE_TABLE}"
}
