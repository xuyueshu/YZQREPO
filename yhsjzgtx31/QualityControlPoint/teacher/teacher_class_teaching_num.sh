...
cd `dirname $0`
source ./../config.sh
exec_dir teacher_class_studnet_num

HIVE_DB=assurance
HIVE_TABLE=teacher_class_studnet_num
...
function create_table() {
    ...
    fn_log "创建表——任课学生数：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 任课学生数表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            (
                ...
                a.code as teacher_no,
                a.name as teacher_name,
                sum(a.student_number) as num
                from
                model.teacher_course_info a
                where a.semester_year in
                (select max(s.semester_year) from model.teacher_course_info s)
                group by  a.semester_year,
                a.semester,
                a.code,
                a.name
...
function export_table_new() {
    ...
    --columns 'data_no,data_name,teacher_name,teacher_no,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_teacher_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
     clear_mysql_data "update assurance.im_quality_teacher_data_info set is_new = 'YES' where data_no = '${DATA_NO}' and data_time= '${DATE_TIME}' "
    fn_log "导出数据-- 任课学生数表:${HIVE_DB}.${TARGET_TABLE}"
}
