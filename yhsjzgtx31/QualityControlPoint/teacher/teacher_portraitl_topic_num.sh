...
cd `dirname $0`
source ./../config.sh
exec_dir teacher_portraitl_topic_num

HIVE_DB=assurance
HIVE_TABLE=teacher_portraitl_topic_num
...
function create_table() {
    ...
    fn_log "创建表——参与纵向课题数量：${HIVE_DB}.${HIVE_TABLE}"
}

#参与的国家级纵向课题数+参与的省部级纵向课题数+参与的院校级纵向课题数
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 ——参与纵向课题数量表：${HIVE_DB}.${HIVE_TABLE}"
}
