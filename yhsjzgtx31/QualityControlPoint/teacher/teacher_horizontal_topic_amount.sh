...
cd `dirname $0`
source ./../config.sh
exec_dir teacher_horizontal_topic_amount

HIVE_DB=assurance
HIVE_TABLE=teacher_horizontal_topic_amount
TARGET_TABLE=im_quality_teacher_data_info
DATA_NO=SZ_CYHXKTDKE
...
function create_table() {
    ...
    fn_log "创建表——参与横向课题到款额：${HIVE_DB}.${HIVE_TABLE}"
}

#scientific_project_personnel_info 这个表范围比较大，需要加上 is_sub_topic='1' 这个条件
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 参与横向课题到款额表：${HIVE_DB}.${HIVE_TABLE}"
}
