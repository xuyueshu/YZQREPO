...
cd `dirname $0`
source ./../config.sh
exec_dir teacher_participation_book_num

HIVE_DB=assurance
HIVE_TABLE=teacher_participation_book_num
...
function create_table() {
    ...
    fn_log "创建表——参编教材数量：${HIVE_DB}.${HIVE_TABLE}"
}

#参编的校本教材数+参编的普通公开出版教材数+参编的国家规划教材数
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            (
                ...
                count(a.textbook_code) as num
                from
               model.teacher_textbook_personnel_info a
               where a.write_type=2 and a.semester_year != ''
               group by a.semester_year,a.code, a.name
             ) a,
            ...
            "
    fn_log "导入数据 —— 参编教材数量：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            (
               ...
               (select max(s.semester_year) from model.teacher_textbook_personnel_info s)
               group by a.semester_year,a.code, a.name
             ) a
             assurance.im_quality_data_base_info c
              where c.data_no ='${DATA_NO}'
...
function export_table_new() {
    ...
    --columns 'data_no,data_name,teacher_name,teacher_no,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_teacher_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
     clear_mysql_data "update assurance.im_quality_teacher_data_info set is_new = 'YES' where data_no = '${DATA_NO}' and data_time= '${DATE_TIME}' "
    fn_log "导出数据-- 参与横向课题到款额表:${HIVE_DB}.${TARGET_TABLE}"
}
