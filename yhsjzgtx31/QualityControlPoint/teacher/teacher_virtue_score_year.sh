...
cd `dirname $0`
source ./../config.sh
exec_dir teacher_virtue_score_year

HIVE_DB=assurance
HIVE_TABLE=teacher_virtue_score_year
...
function create_table() {
    ...
    fn_log "创建表——年度考核得分表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                   ...
                   a.code as teacher_name,
                   a.name as teacher_no,
                   c.data_cycle as data_cycle,
                   c.data_type as data_type,
                   a.semester_year  as  data_time ,
                   a.assessment_score as data_value,
                   'NO' as is_new,
                FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
            from
            model.teacher_growing_assessment_info a,
            assurance.im_quality_data_base_info c  where c.data_no ='${DATA_NO}'


            "
    fn_log "导入数据 —— 年度考核得分表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            (select max(s.semester_year) from model.teacher_growing_assessment_info s )
...
function export_table_new() {
    ...
    --columns 'data_no,data_name,teacher_name,teacher_no,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_teacher_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
     clear_mysql_data "update assurance.im_quality_teacher_data_info set is_new = 'YES' where data_no = '${DATA_NO}' and data_time= '${DATE_TIME}' "
    fn_log "导出数据-- 年度考核得分表:${HIVE_DB}.${TARGET_TABLE}"
}
