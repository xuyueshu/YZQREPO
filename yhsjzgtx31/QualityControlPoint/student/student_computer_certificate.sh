...
cd `dirname $0`
source ./../config.sh
exec_dir student_computer_certificate

HIVE_DB=assurance
HIVE_TABLE=student_computer_certificate
...
function create_table() {
    ...
    fn_log "创建表——计算机证书考取数表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 (
                      ...
                      count(case when a.name like '%计算机%' then a.paper_code end) as num
                      from
    ...
    "
    fn_log "导入数据 —— 计算机证书考取数表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
    ...
    fn_log "导出数据--计算机证书考取数表:${HIVE_DB}.${TARGET_TABLE}"
}
