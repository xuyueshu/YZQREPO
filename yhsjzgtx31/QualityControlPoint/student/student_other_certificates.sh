...
cd `dirname $0`
source ./../config.sh
exec_dir student_other_certificates

HIVE_DB=assurance
HIVE_TABLE=student_other_certificates
...
function create_table() {
    ...
    fn_log "创建表——其他证（照）考取数量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    ...
    "
    fn_log "导入数据 —— 其他证（照）考取数量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table() {
    ...
    fn_log "导出数据--其他证（照）考取数量表:${HIVE_DB}.${TARGET_TABLE}"
}
