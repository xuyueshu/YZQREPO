...
cd `dirname $0`
source ./../config.sh
exec_dir student_network_duration

HIVE_DB=assurance
HIVE_TABLE=student_network_duration
...
function create_table() {
    ...
    fn_log "创建表——日均上网时长表：${HIVE_DB}.${HIVE_TABLE}"
}
function import_table() {
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
             (
                    ...
                    cast(( sum(a.workday_netPlay_hours+a.weekend_netPlay_hours) /
                           count(0)
                           ) as decimal(9,2)) as num
                    from
                    app.student_netPlay_record a
    ...
    "
    fn_log "导入数据 —— 日均上网时长表：${HIVE_DB}.${HIVE_TABLE}"
}
           ...
           select
            ...
            from
             (
                    ...
                    (select max(s.semester_year) from app.student_netPlay_record s)
                    group by a.semester_year,
                    a.semester,
                    a.statistics_month,
                    a.code ,
...
function export_table_new() {
    ...
    --columns 'data_no,data_name,student_no,student_name,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_student_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
     clear_mysql_data "update assurance.im_quality_student_data_info set is_new = 'YES' where data_no = '${DATA_NO}' and data_time= '${DATE_TIME}' "
    fn_log "导出数据--日均上网时长表:${HIVE_DB}.${TARGET_TABLE}"
}
