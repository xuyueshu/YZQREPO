...
cd `dirname $0`
source ./../config.sh
exec_dir student_class_ranking

HIVE_DB=assurance
HIVE_TABLE=student_class_ranking
...
function create_table() {
    ...
    fn_log "创建表——班级排名表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
                   ...
                   row_number() over(partition by  a.semester_year,a.semester,a.class_code order by a.score desc)  as data_value,
                   'NO' as is_new,
                   FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                   ) AS create_time
    ...
    "
    fn_log "导入数据 —— 班级排名表：${HIVE_DB}.${HIVE_TABLE}"
}
         ...
         select
            ...
            from
                 (
                      ...
                      (select max(s.semester_year) from model.student_score_record s)
                      group by
                      a.semester_year,
                      a.semester,
                      a.code,
...
#     clear_mysql_data "update assurance.im_quality_student_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
#     clear_mysql_data "update assurance.im_quality_student_data_info set is_new = 'YES' where data_no = '${DATA_NO}' and data_time= '${DATE_TIME}' "
#    fn_log "导出数据--班级排名表:${HIVE_DB}.${TARGET_TABLE}"
    DATE_TIME=`hive -e "select max(data_time) from ${HIVE_DB}.${HIVE_TABLE} " `
    clear_mysql_data "delete from im_quality_student_data_info
            where data_no = '${DATA_NO}' and data_time= '${DATE_TIME}';"
    sqoop export --connect ${MYSQL_URL} --username ${MYSQL_USERNAME} --password ${MYSQL_PASSWORD} \
