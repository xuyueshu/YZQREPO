...
cd `dirname $0`
source ./../config.sh
exec_dir student_test_of_physical_strength

HIVE_DB=assurance
HIVE_TABLE=student_test_of_physical_strength
...
function create_table() {
    ...
    fn_log "创建表——体测成绩表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
             (
                      ...
                      sum(a.score) as num
                      from
                      model.student_physical_test_record a
                      left join
    ...
    "
    fn_log "导入数据 —— 体测成绩表：${HIVE_DB}.${HIVE_TABLE}"
}
            ...
            from
             (
                      ...
                      where b.in_school='1' and b.status='1' and
                       a.semester_year in
                      (select max(s.semester_year) from model.student_physical_test_record s)
                      group by a.semester_year,
                      a.semester,
                      a.code,
                      b.name
...
function export_table_new() {
    ...
    --columns 'data_no,data_name,student_no,student_name,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_student_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
     clear_mysql_data "update assurance.im_quality_student_data_info set is_new = 'YES' where data_no = '${DATA_NO}' and data_time= '${DATE_TIME}' "
    fn_log "导出数据--体测成绩表:${HIVE_DB}.${TARGET_TABLE}"
}
