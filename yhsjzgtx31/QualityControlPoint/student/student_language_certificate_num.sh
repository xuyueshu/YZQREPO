...
cd `dirname $0`
source ./../config.sh
exec_dir student_language_certificate_num

HIVE_DB=assurance
HIVE_TABLE=student_language_certificate_num
...
function create_table() {
    ...
    fn_log "创建表——语言证书考取数表：${HIVE_DB}.${HIVE_TABLE}"
}

#3.0 [student_papers] 中 证书名：
#机动车驾驶证/中国精算师/金融师/会计从业资格证/大学英语四级考试（CET-4）/大学英语六级考试（CET-6）/计算机二级
#可能还有其他的证书
function import_table() {
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 (
                      ...
                      count(case when a.name like '%CET%' then a.paper_code end) as num
                      from
                      model.student_papers a
                      left join
    ...
    "
    fn_log "导入数据 —— 语言证书考取数表：${HIVE_DB}.${HIVE_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 (
                      ...
                      (select max(s.semester_year) from model.student_papers s)
                      group by a.semester_year,
                      a.semester,
                      a.code,
                      b.name
...
function export_table_new() {
    ...
    --columns 'data_no,data_name,student_no,student_name,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_student_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
     clear_mysql_data "update assurance.im_quality_student_data_info set is_new = 'YES' where data_no = '${DATA_NO}' and data_time= '${DATE_TIME}' "
    fn_log "导出数据--语言证书考取数表:${HIVE_DB}.${TARGET_TABLE}"
}
