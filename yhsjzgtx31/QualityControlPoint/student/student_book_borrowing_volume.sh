...
cd `dirname $0`
source ./../config.sh
exec_dir student_book_borrowing_volume

HIVE_DB=assurance
HIVE_TABLE=student_book_borrowing_volume
...
function create_table() {
    ...
    fn_log "创建表——图书借阅量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    ...
    "
    fn_log "导入数据 —— 图书借阅量表：${HIVE_DB}.${HIVE_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
               (
                      ...
                      count(a.book_code) as num
                      from
                      app.teacher_student_book_lending_record a
                      left join
                      app.basic_student_info b
                      on a.code=b.code
                      where b.in_school='1' and b.status='1'
                      and a.semester_year in
                      (select max(s.semester_year) from app.teacher_student_book_lending_record s)
                      group by a.semester_year,
                      a.semester,
                      a.code,
                      b.name
...
function export_table_new() {
    ...
    --columns 'data_no,data_name,student_no,student_name,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_student_data_info set is_new = 'NO' where data_no = '${DATA_NO}';"
     clear_mysql_data "update assurance.im_quality_student_data_info set is_new = 'YES' where data_no = '${DATA_NO}' and data_time= '${DATE_TIME}' "
    fn_log "导出数据--图书借阅量表:${HIVE_DB}.${TARGET_TABLE}"
}
