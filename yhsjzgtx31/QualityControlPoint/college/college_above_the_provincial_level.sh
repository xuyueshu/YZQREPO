...
cd `dirname $0`
source ./../config.sh
exec_dir college_above_the_provincial_level

HIVE_DB=assurance
HIVE_TABLE=college_above_the_provincial_level
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                ...
                a.province_above_prize_num as data_value,
                'NO' as is_new,
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                      (
                       select count(1) as prize_winning,semester_year from  model.teacher_awards_info t where awards_type in ('1','2') group by semester_year
