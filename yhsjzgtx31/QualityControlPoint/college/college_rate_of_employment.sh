...
cd `dirname $0`
source ./../config.sh
exec_dir college_rate_of_employment

HIVE_DB=assurance
HIVE_TABLE=college_rate_of_employment
...
function create_table() {
    ...
    fn_log "创建表——就业率：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
               (
               select
               concat(cast(sum(employment_rate) as decimal(9,2)),'','%') as n,
               a.semester_year from app.student_graduate_employment_count a
               where a.semester_year!=''
               group by semester_year
               ) a,
               assurance.im_quality_data_base_info c
                where c.data_no = '${DATA_NO}'
            "
    fn_log "导入数据 —— 就业率：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                       ...
                       concat(cast(sum(employment_rate) as decimal(9,2)),'','%') as n,
                       a.semester_year
                       from app.student_graduate_employment_count a
                       where a.semester_year in
                       (select max(semester_year) from app.student_graduate_employment_count)
                       and a.semester_year!=''
                       group by semester_year
                       )a,
                      assurance.im_quality_data_base_info c
