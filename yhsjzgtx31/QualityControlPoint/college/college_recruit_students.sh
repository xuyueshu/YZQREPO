...
cd `dirname $0`
source ./../config.sh
exec_dir college_recruit_students

HIVE_DB=assurance
HIVE_TABLE=college_recruit_students
...
function create_table() {
    ...
    fn_log "创建表——实际报到人数：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 (
                     select
                     cast(sum(a.actual_enroll_student_count) as int) as count_p,
                     a.semester_year
                     from
            ...
            "
    fn_log "导入数据 —— 实际报到人数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
