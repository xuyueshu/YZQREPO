...
cd `dirname $0`
source ./../config.sh
exec_dir college_total_number_of_papers_published

HIVE_DB=assurance
HIVE_TABLE=college_total_number_of_papers_published
...
function create_table() {
    ...
    fn_log "创建表——发表论文总数 ：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 发表论文总数 ：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                      ...
                      FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                      app.college_scientific_count a,
                      assurance.im_quality_data_base_info c
