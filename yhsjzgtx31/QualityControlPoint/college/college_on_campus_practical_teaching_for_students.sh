...
cd `dirname $0`
source ./../config.sh
exec_dir college_on_campus_practical_teaching_for_students

HIVE_DB=assurance
HIVE_TABLE=college_on_campus_practical_teaching_for_students
TARGET_TABLE=im_quality_data_info
DATA_NAME=生均校内实践教学工位数
...
function create_table() {
    ...
    ) COMMENT '学校生均校内实践教学工位数表'
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——学校生均校内实践教学工位数表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                      (
                       select practice_seat_avg,semester_year from app.college_assets_student_avg t
