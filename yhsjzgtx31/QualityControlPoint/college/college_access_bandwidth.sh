...
cd `dirname $0`
source ./../config.sh
exec_dir college_access_bandwidth

HIVE_DB=assurance
HIVE_TABLE=college_access_bandwidth
...
function create_table() {
    ...
    fn_log "创建表——学校出口带宽表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
             select
                ...
                a.export_broadband as data_value,
                'NO' as is_new,
            ...
            "
    fn_log "导入数据 —— 学校出口带宽表：${HIVE_DB}.${HIVE_TABLE}"
}
