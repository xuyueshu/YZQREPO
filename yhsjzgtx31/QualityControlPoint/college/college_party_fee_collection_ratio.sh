...
cd `dirname $0`
source ./../config.sh
exec_dir college_party_fee_collection_ratio
...
#DATA_NAME=党费收缴比例
DATA_NO=XY_DFSJBL
HIVE_TABLE=college_party_fee_collection_ratio
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                ...
                a.party_cont as data_value,
                'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            FROM
              (
             select
              CONCAT(nvl(cast(sum(a.party_payed_count)/sum(a.teacher_party_count+a.student_party_count)*100 as decimal(9,2)),0),'','%')
              as party_cont,
              a.semester_year
             from app.college_party_info a
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            FROM
             ...
             group by a.semester_year
              ...
              where c.data_no = '${DATA_NO}'
               and a.semester_year in (select max(semester_year) from app.college_party_info)
         "
}
