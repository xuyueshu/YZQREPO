...
cd `dirname $0`
source ./../config.sh
exec_dir college_teacher_num

HIVE_DB=assurance
HIVE_TABLE=college_teacher_num
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                ...
                count(a.code) as data_value,
                'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                model.basic_teacher_info a,assurance.im_quality_data_base_info c
              where c.data_no = '${DATA_NO}'
            group by a.semester_year,c.data_no,
            c.data_name,c.data_cycle,c.data_type
