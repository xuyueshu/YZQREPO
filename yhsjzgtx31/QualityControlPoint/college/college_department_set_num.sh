...
cd `dirname $0`
source ./../config.sh
exec_dir college_department_set_num

HIVE_DB=assurance
TARGET_TABLE=im_quality_data_info
DATA_NO=XY_ZZJGSZSL
HIVE_TABLE=college_department_set_num
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            SELECT
              ...
              count(a.dept_code) as data_value,
              'NO' as is_new,
              FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            FROM
              app.major_abroad_communication_count a,assurance.im_quality_data_base_info c
