...
cd `dirname $0`
source ./../config.sh
exec_dir college_media_coverage

HIVE_DB=assurance
HIVE_TABLE=college_media_coverage
...
function create_table() {
    ...
    fn_log "创建表——学校各类媒体报道数量表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
          select
                ...
                a.media_reports_num as data_value,
                'NO' as is_new,
            ...
            "
    fn_log "导入数据 —— 学校各类媒体报道数量表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
