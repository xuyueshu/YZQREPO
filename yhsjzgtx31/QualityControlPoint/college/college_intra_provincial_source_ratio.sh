...
cd `dirname $0`
source ./../config.sh
exec_dir college_intra_provincial_source_ratio

HIVE_DB=assurance
HIVE_TABLE=college_intra_provincial_source_ratio
...
function create_table() {
    ...
    fn_log "创建表——省内生源比例：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                ...
                concat(a.count_s,':',b.count_s) as data_value,
                 'NO' as is_new,
            ...
            from
                (
                       ...
                       b.province  in (select province from model.college_basic_info limit 1)
                       group by a.semester_year,a.semester
                   ) a left join
                  (
                      select count(1) count_s,
                      a.semester_year,
                      a.semester
                      from app.basic_semester_student_info a
                      group by a.semester_year,a.semester
                  )b
            ...
            "
    fn_log "导入数据 —— 省内生源比例：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
...
function export_table() {
    ...
    fn_log "导出数据--省内生源比例:${HIVE_DB}.${TARGET_TABLE}"
}
