...
HIVE_DB=assurance
HIVE_TABLE=college_exchange_of_visits
...
function create_table() {
    ...
    fn_log "创建表——学校到款额表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         ...
         select
            ...
            from
              model.major_social_work,assurance.im_quality_data_base_info c
               where  c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校到款额表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
              model.major_social_work,assurance.im_quality_data_base_info c
               where  c.data_name = '${DATA_NAME}' order by data_time desc limit 1
         "
}
