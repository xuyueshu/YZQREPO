...
cd `dirname $0`
source ./../config.sh
exec_dir college_party_student_num_radio
...
#DATA_NAME=党员学生比例
DATA_NO=XY_DYXSSL
HIVE_TABLE=college_party_student_num_radio
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
            ...
            FROM
             (select
             CONCAT(nvl(cast(sum(a.student_party_count)/sum(a.teacher_party_count+a.student_party_count)*100 as decimal(9,2)),0),'','%') as party_cont,
             a.semester_year
