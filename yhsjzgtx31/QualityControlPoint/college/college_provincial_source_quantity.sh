...
cd `dirname $0`
source ./../config.sh
exec_dir college_provincial_source_quantity

HIVE_DB=assurance
HIVE_TABLE=college_provincial_source_quantity
...
function create_table() {
    ...
    fn_log "创建表——省内生源数量：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         ...
         from
           ...
           where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 省内生源数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
...
function export_table() {
    ...
    fn_log "导出数据--省内生源数量:${HIVE_DB}.${TARGET_TABLE}"
}
