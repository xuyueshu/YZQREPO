...
cd `dirname $0`
source ./../config.sh
exec_dir college_number_of_enrollment_majors

HIVE_DB=assurance
HIVE_TABLE=college_number_of_enrollment_majors
...
function create_table() {
    ...
    fn_log "创建表——招生专业数量：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                ...
                a.enroll_student_major_count as data_value,
                 'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                 app.major_total_info a,assurance.im_quality_data_base_info c
                  where c.data_name = '${DATA_NAME}' and a.type='1'
            "
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    select
                      ...
                      FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                       (select a.enroll_student_major_count,a.semester_year from  app.major_total_info a
