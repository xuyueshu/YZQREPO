...
cd `dirname $0`
source ./../config.sh
exec_dir college_international_cooperation_and_exchange

HIVE_DB=assurance
HIVE_TABLE=college_international_cooperation_and_exchange
TARGET_TABLE=im_quality_data_info
DATA_NAME=国际合作交流活动数量
...
function create_table() {
    ...
    ) COMMENT '学校国际合作交流活动数量表'
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——学校国际合作交流活动数量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 (select semester_year,sum(activity_num) as num from model.college_international_cooperation
                 group by semester_year) a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校国际合作交流活动数量表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                c.data_type as data_type,
                a.semester_year as data_time
                a.activity_num as data_value,
                   'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                 model.college_international_cooperation a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}' and a.semester_year in (select max(semester_year) from model.college_international_cooperation)
         "
}
