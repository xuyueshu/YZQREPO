...
cd `dirname $0`
source ./../config.sh
exec_dir college_actual_reporting_rate

HIVE_DB=assurance
HIVE_TABLE=college_actual_reporting_rate
...
function create_table() {
    ...
    fn_log "创建表——实际报到率：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                (
                select
                nvl(cast(sum(a.actual_enroll_student_count)/sum(a.plan_enroll_student_count) as decimal(9,2)),0) as count_p,
                a.semester_year  from model.major_enroll_student a
            ...
            "
    fn_log "导入数据 —— 实际报到率：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
