...
cd `dirname $0`
source ./../config.sh
exec_dir college_school_assets

HIVE_DB=assurance
HIVE_TABLE=college_school_assets
...
function create_table() {
    ...
    fn_log "创建表——学校全校资产总值表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
          select
                ...
                a.assets_total as data_value,
                 'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from model.college_basic_info a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校全校资产总值表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                      (
                       select assets_total,semester_year from model.college_basic_info t
