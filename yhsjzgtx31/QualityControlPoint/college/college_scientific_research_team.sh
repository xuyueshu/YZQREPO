...
cd `dirname $0`
source ./../config.sh
exec_dir college_topic_total

HIVE_DB=assurance
HIVE_TABLE=college_scientific_research_team
...
function create_table() {
    ...
    fn_log "创建表——学校科研团队数量表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
           select
                ...
                a.scientific_team_num as data_value,
                  'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
              app.college_scientific_count a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校科研团队数量表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
              app.college_scientific_count a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}' and a.semester_year in (select max(semester_year) from app.college_scientific_count)
         "
}
