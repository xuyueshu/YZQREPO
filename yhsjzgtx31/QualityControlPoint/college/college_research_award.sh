...
cd `dirname $0`
source ./../config.sh
exec_dir college_research_award

HIVE_DB=assurance
HIVE_TABLE=college_research_award
...
function create_table() {
    ...
    fn_log "创建表——学校科研获奖数量表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
          select
                ...
                a.prize_winning_num as data_value,
                  'NO' as is_new,
            ...
            "
    fn_log "导入数据 —— 学校教科研获奖数量表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
