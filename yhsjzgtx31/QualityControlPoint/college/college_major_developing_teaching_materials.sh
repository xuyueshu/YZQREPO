...
cd `dirname $0`
source ./../config.sh
exec_dir college_major_developing_teaching_materials

HIVE_DB=assurance
HIVE_TABLE=college_major_developing_teaching_materials
TARGET_TABLE=im_quality_data_info
DATA_NO=XY_QYYYXQHZGTKFJCS
...
function create_table() {
    ...
    ) COMMENT '专业拥有校企合作共同开发教材数 '
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——专业拥有校企合作共同开发教材数 ：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
              select distinct
                    ...
                    from(
                       select sum(textbook_count) as n,semester_year as t from
                       app.major_chief_editor_textbook
                       where textbooks_type='XQHZ'
                       group by semester_year
                     )a,
                      assurance.im_quality_data_base_info c
                      where c.data_no = '${DATA_NO}'
            "
    fn_log "导入数据 —— 专业拥有校企合作共同开发教材数 ：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
