...
cd `dirname $0`
source ./../config.sh
exec_dir college_leader_num

HIVE_DB=assurance
HIVE_TABLE=college_leader_num
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                SELECT
                  ...
                  count(a.teacher_code) as data_value,
                  'NO' as is_new,
                  FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
                FROM
                  model.college_admin_team_info a,assurance.im_quality_data_base_info c
                where c.data_no = '${DATA_NO}' and a.leader_type = 'LDBZ'
                GROUP BY
                  a.semester_year,
                  c.data_no,
                  c.data_name,
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                where
                  c.data_no = '${DATA_NO}' and a.leader_type = 'LDBZ'
                GROUP BY
                  a.semester_year,
                  c.data_no,
                  c.data_name,
