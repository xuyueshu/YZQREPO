...
cd `dirname $0`
source ./../config.sh
exec_dir college_student_graduation_rate
HIVE_DB=assurance
HIVE_TABLE=college_student_graduation_rate
                ...
                from
                  (
                    SELECT
                      semester_year,
                      cast(
                        avg(graduation_rate) as DECIMAL(6, 2)
                      ) as num
                    from
                      app.student_graduate_employment_count
                      where semester_year!=''
