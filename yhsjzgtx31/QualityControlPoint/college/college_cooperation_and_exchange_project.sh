...
cd `dirname $0`
source ./../config.sh
exec_dir college_cooperation_and_exchange_project

HIVE_DB=assurance
HIVE_TABLE=college_cooperation_and_exchange_project
...
function create_table() {
    ...
    fn_log "创建表——学校合作交流项目数表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         ...
         select
            ...
            from
                 (select semester_year,sum(project_num) as num from model.college_international_cooperation
                 group by semester_year) a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
                
            "
    fn_log "导入数据 —— 学校合作交流项目数表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                c.data_type as data_type,
                a.semester_year as data_time
                sum(a.project_num) as data_value,
                   'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                 model.college_international_cooperation a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
                and a.semester_year in (select max(semester_year) from model.college_international_cooperation)
                 group by a.semester_year,c.data_no,c.data_name,c.data_cycle,c.data_type
         "
}
