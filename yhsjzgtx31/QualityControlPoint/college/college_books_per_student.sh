...
cd `dirname $0`
source ./../config.sh
exec_dir college_books_per_student

HIVE_DB=assurance
HIVE_TABLE=college_books_per_student
...
function create_table() {
    ...
    fn_log "创建表——学校生均图书表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                c.data_type as data_type,
                a.semester_year as data_time,
                a.book_avg as data_value,
                    'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from app.college_assets_student_avg a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}' and a.semester_year in
                (select max(semester_year) from app.college_assets_student_avg)
         "
}
