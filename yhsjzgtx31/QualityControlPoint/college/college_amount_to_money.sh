...
cd `dirname $0`
source ./../config.sh
exec_dir college_amount_to_money

HIVE_DB=assurance
HIVE_TABLE=college_amount_to_money
...
function create_table() {
    ...
    fn_log "创建表——学校来访交流数量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
select
    ...
    (a.enterprise_visit_communication_num+a.college_visit_communication_num+a.other_visit_communication_num) as data_value,
    'NO' as is_new,
    FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
from  model.college_social_influence_count a,assurance.im_quality_data_base_info c
    where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校来访交流数量表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                      (
                       select (a.enterprise_visit_communication_num+a.college_visit_communication_num+a,other_visit_communication_num) as count_p,a.semester_year
                from  model.college_social_influence_count a where a.semester_year in (select max(semester_year) from model.college_social_influence_count)
                      ) a,
