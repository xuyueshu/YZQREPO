...
cd `dirname $0`
source ./../config.sh
exec_dir college_party_teacher_num_radio
...
#DATA_NAME=党员教师比例
DATA_NO=XY_DYJSSL
HIVE_TABLE=college_party_teacher_num_radio
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
          select
            ...
            FROM
             (select
             CONCAT(cast(sum(a.teacher_party_count)/sum(a.teacher_party_count+a.student_party_count)*100 as decimal(9,2)),'','%') as party_cont,
             a.semester_year
