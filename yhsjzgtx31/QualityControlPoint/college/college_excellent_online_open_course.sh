...
cd `dirname $0`
source ./../config.sh
exec_dir college_excellent_online_open_course
HIVE_DB=assurance
HIVE_TABLE=college_excellent_online_open_course
TARGET_TABLE=im_quality_data_info
DATA_NAME=精品在线开放课程数量
...
function create_table() {
    ...
    ) COMMENT '学校精品在线开放课程数量表'
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——学校精品在线开放课程数量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         ...
         select
                ...
                sum(a.course_count) as data_value,
                 'NO' as is_new,
            ...
            from
                ...
                group by c.data_no,c.data_name,c.data_cycle,
                c.data_type,a.semester_year,a.semester
            "
    fn_log "导入数据 —— 学校精品在线开放课程数量表：${HIVE_DB}.${HIVE_TABLE}"
}
