...
cd `dirname $0`
source ./../config.sh
exec_dir college_planned_enrollment

HIVE_DB=assurance
HIVE_TABLE=college_planned_enrollment
...
function create_table() {
    ...
    fn_log "创建表——计划招生人数：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                ...
                cast(sum(a.plan_enroll_student_count) as int)  as data_value,
                 'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                  model.major_enroll_student a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
                group by data_no,data_name,data_cycle,data_type,semester_year,create_time
            "
    fn_log "导入数据 —— 计划招生人数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                       (
                           select cast(sum(a.plan_enroll_student_count) as int) as t_count,semester_year from  model.major_enroll_student a
                           where a.semester_year in (select max(semester_year) from model.major_enroll_student )
