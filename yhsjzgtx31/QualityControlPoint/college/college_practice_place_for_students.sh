...
cd `dirname $0`
source ./../config.sh
exec_dir college_practice_place_for_students

HIVE_DB=assurance
HIVE_TABLE=college_practice_place_for_students
...
function create_table() {
    ...
    fn_log "创建表——学校生均实践场所表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function export_table_new(){
    ...
    clear_mysql_data "delete from im_quality_data_info where data_name = '${DATA_NAME}' and data_time = '${DATE_TIME}';"
    sqoop export --connect ${MYSQL_URL} --username ${MYSQL_USERNAME} --password ${MYSQL_PASSWORD} \
    --table ${TARGET_TABLE} --export-dir  ${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE} \
    --input-fields-terminated-by '\0001' --input-null-string '\\N' --input-null-non-string '\\N' \
    --null-string '\\N' --null-non-string '\\N' \
