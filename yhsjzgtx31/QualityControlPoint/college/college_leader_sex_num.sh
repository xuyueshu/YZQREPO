...
cd `dirname $0`
source ./../config.sh
exec_dir college_leader_sex_num

HIVE_DB=assurance
TARGET_TABLE=im_quality_data_info

DATA_NO=XY_LDBZXBBL
HIVE_TABLE=college_leader_sex_num
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                SELECT
                  ...
                  cast((b.num / d.num) as decimal(6,2)) as data_value,
                  'NO' as is_new,
                  FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
                FROM
                  (
                    select
                      ...
                      a.semester_year
                    from
                      model.college_admin_team_info a
                    where
                      a.sex = '1' and a.leader_type = 'LDBZ'
                    group by
                      a.semester_year
                  ) b,
                  (
                    ...
                    from
                      model.college_admin_team_info a
                    where
                      a.sex = '2' and a.leader_type = 'LDBZ'
                    group by
                      a.semester_year
                  ) d,
                  assurance.im_quality_data_base_info c
                where
                  c.data_no = '${DATA_NO}' and b.semester_year = d.semester_year
         "
}
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                where
                  c.data_no = '${DATA_NO}'
                  and b.semester_year = d.semester_year
                ORDER BY
                  data_time DESC
                LIMIT
                  1
