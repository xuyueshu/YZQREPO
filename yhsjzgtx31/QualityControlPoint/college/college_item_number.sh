...
cd `dirname $0`
source ./../config.sh
exec_dir college_item_number

HIVE_DB=assurance
HIVE_TABLE=college_item_number
...
function create_table() {
    ...
    fn_log "创建表——学校项目数表：${HIVE_DB}.${HIVE_TABLE}"
}
#只有实训项目数和科研项目数据 学校各类项目总数-科研项目数 所以统计 实训项目数
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                ...
                count(distinct a.project_name) as data_value,
                 'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
             from
                model.major_trainingProject_detailed a,
                assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
                group by c.data_no,c.data_name,c.data_cycle,c.data_type,
                a.semester_year,a.semester
            "
    fn_log "导入数据 —— 学校项目数表：${HIVE_DB}.${HIVE_TABLE}"
}
