...
cd `dirname $0`
source ./../config.sh
exec_dir college_proportion_of_single_enrollment_types

HIVE_DB=assurance
HIVE_TABLE=college_proportion_of_single_enrollment_types
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 (
                      select
                      nvl(cast(sum(a.single_recruit_num)/sum(a.student_num) as decimal(9,2)),0) as count_p,
                      a.semester_year
                      from
                      app.college_enrolment_method_count a
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                      (
                          ...
                          where a.semester_year in (select max(semester_year) from app.college_enrolment_method_count)
                          group by a.semester_year
                      ) a,
                      assurance.im_quality_data_base_info c
                      where c.data_name = '${DATA_NAME}'
