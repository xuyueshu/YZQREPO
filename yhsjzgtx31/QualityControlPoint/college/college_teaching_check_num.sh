...
cd `dirname $0`
source ./../config.sh
exec_dir college_teaching_check_num

HIVE_DB=assurance
HIVE_TABLE=college_teaching_check_num
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                (
                 select
                 cast(sum(teaching_inspections_count) as int) as count_t,semester_year from  model.college_quality_info
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                FROM
                (
                 select cast(sum(teaching_inspections_count) as int) as count_t,semester_year from  model.college_quality_info
                   group by semester_year order by semester_year desc limit 1
                 )a,assurance.im_quality_data_base_info c
                where c.data_no = '${DATA_NO}'
...
#export_table
##以后的每次执行
##执行create_table，import_table_new，export_table_new
##import_table_new
##export_table_new
#finish
