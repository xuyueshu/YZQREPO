...
cd `dirname $0`
source ./../config.sh
exec_dir college_research_funding

HIVE_DB=assurance
HIVE_TABLE=college_research_funding
...
function create_table() {
    ...
    fn_log "创建表——学校科研到款表：${HIVE_DB}.${HIVE_TABLE}"
}
...
select
    ...
    cast(a.amount/10000 as decimal(9,2)) as data_value,
    'NO' as is_new,
    FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
from
    (select sum(a.amount) as amount,a.semester_year from model.teacher_project_count a
    group by a.semester_year
    ) a,
    assurance.im_quality_data_base_info c
    where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校科研到款表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                c.data_cycle as data_cycle,
                c.data_type as data_type,
                a.semester_year as data_time,课题总数
                a.amount as data_value,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                  (select sum(a.amount) as amount,a.semester_year from app.teacher_project_count a
                  where a.semester_year in (select max(semester_year) from app.teacher_project_count)
                  group by a.semester_year
                  ) a,
                  assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
