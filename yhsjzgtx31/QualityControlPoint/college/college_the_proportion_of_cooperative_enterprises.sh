...
cd `dirname $0`
source ./../config.sh
exec_dir college_the_proportion_of_cooperative_enterprises

HIVE_DB=assurance
HIVE_TABLE=college_the_proportion_of_cooperative_enterprises
TARGET_TABLE=im_quality_data_info
DATA_NO=XY_YHZQYDZYSZZYSZZSBL
...
function create_table() {
    ...
    ) COMMENT '有合作企业的专业数占专业设置总数比例'
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——有合作企业的专业数占专业设置总数比例：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                      ...
                      concat(cast(a.num/b.num *100 as decimal(9,2)),'','%') as data_value,
                      'NO' as is_new,
                      FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                    (
                    select
                    count(major_code) as num,
                    semester_year
                    from
                    model.major_donation_major_count
                    where cooperation_company_count>0
            ...
            "
    fn_log "导入数据 —— 有合作企业的专业数占专业设置总数比例：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
