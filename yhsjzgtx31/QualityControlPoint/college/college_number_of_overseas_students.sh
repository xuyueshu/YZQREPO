...
cd `dirname $0`
source ./../config.sh
exec_dir college_number_of_overseas_students

HIVE_DB=assurance
HIVE_TABLE=college_number_of_overseas_students
...
function create_table() {
    ...
    fn_log "创建表——留学生人数：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 (select
                 cast(sum(a.overseas_student_count) as int) as count_p,a.semester_year from model.major_enroll_student a
                 group  by a.semester_year
                 )a,assurance.im_quality_data_base_info c
                where c.data_no = '${DATA_NO}'
            "
    fn_log "导入数据 —— 留学生人数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                 (select cast(sum(a.overseas_student_count) as int) as count_p,a.semester_year from model.major_enroll_student a
