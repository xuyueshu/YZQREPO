...
cd `dirname $0`
source ./../config.sh
exec_dir college_students_with_multimedia_classroom_seats

HIVE_DB=assurance
HIVE_TABLE=college_students_with_multimedia_classroom_seats
TARGET_TABLE=im_quality_data_info
DATA_NAME=百名学生配多媒体教室座位数
...
function create_table() {
    ...
    ) COMMENT '学校百名学生配多媒体教室座位数表'
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——学校百名学生配多媒体教室座位数表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         ...
         select
                ...
                cast(a.multimedia_seat_hundred_avg as decimal(9,0)) as data_value,
                'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from app.college_assets_student_avg a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校百名学生配多媒体教室座位数表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
