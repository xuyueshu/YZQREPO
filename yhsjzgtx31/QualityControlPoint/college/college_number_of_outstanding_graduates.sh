...
cd `dirname $0`
source ./../config.sh
exec_dir college_number_of_outstanding_graduates

HIVE_DB=assurance
HIVE_TABLE=college_number_of_outstanding_graduates
...
function create_table() {
    ...
    fn_log "创建表——优秀毕业生数量：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
               (select count(code) as n,semester_year  from model.major_excellent_graduate a
               group by semester_year
               ) a,
               assurance.im_quality_data_base_info c
                where c.data_no = '${DATA_NO}'
            "
    fn_log "导入数据 —— 优秀毕业生数量：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                       (select count(code) as n,semester_year  from model.major_excellent_graduate a
                       where a.semester_year in (select max(semester_year) from model.major_excellent_graduate)
                        group by semester_year
               )a,
                      assurance.im_quality_data_base_info c
                      where c.data_no = '${DATA_NO}'
