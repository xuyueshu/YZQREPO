...
cd `dirname $0`
source ./../config.sh
exec_dir college_quantity_of_evidence_collection

HIVE_DB=assurance
HIVE_TABLE=college_quantity_of_evidence_collection
TARGET_TABLE=im_quality_data_info
#DATA_NAME=取证数量XY_QZSL
DATA_NO=XY_QZSL
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                ...
                a.student_papers as data_value,
                 'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                  (select count(a.paper_code) as student_papers,a.semester_year from model.student_papers a group by semester_year) a,
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    select
                      ...
                      c.data_type as data_type,
                      a.semester_year as data_time,
                      a.paper_code as data_value,
                      'NO' as is_new,
                      FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                      (
                       select count(a.paper_code) as paper_code,a.semester_year from model.student_papers a group by semester_year
                        order by a.semester_year desc limit 1
                      ) a,
                      assurance.im_quality_data_base_info c
