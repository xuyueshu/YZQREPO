...
cd `dirname $0`
source ./../config.sh
exec_dir college_actual_enrollment

HIVE_DB=assurance
HIVE_TABLE=college_actual_enrollment
...
function create_table() {
    ...
    fn_log "创建表——实际录取人数：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
                ...
                cast(sum(a.actual_enroll_student_count) as int) as data_value,
                 'NO' as is_new,
            ...
            from
                ...
                a.semester_year
            "
    fn_log "导入数据 —— 实际录取人数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                       (
                       select
                       cast(sum(a.actual_enroll_student_count) as int) as t_count,
                       semester_year from  model.major_enroll_student a
                       where a.semester_year in (select max(semester_year) from app.major_total_info )
