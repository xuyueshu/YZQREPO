...
cd `dirname $0`
source ./../config.sh
exec_dir college_integration_project_of_industry_and_education

HIVE_DB=assurance
HIVE_TABLE=college_integration_project_of_industry_and_education
...
function create_table() {
    ...
    fn_log "创建表——产教融合项目数 ：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
              select distinct
                      ...
                      a.production_education_fuse_project_count as data_value,
                      'NO' as is_new,
            ...
            "
    fn_log "导入数据 —— 产教融合项目数 ：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
