...
cd `dirname $0`
source ./../config.sh
exec_dir college_number_of_students_assigned_to_computers

HIVE_DB=assurance
HIVE_TABLE=college_number_of_students_assigned_to_computers
TARGET_TABLE=im_quality_data_info
DATA_NAME=百名学生配教学用计算机数
...
function create_table() {
    ...
    ) COMMENT '学校百名学生配教学用计算机数表'
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——学校百名学生配教学用计算机数表：${HIVE_DB}.${HIVE_TABLE}"
}
