...
cd `dirname $0`
source ./../config.sh
exec_dir college_supervised_instruction_num

HIVE_DB=assurance
HIVE_TABLE=college_supervised_instruction_num
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                FROM(
                 select
                 cast(sum(supervisor_attendance_time) as int) as count_t
                 ,semester_year from model.college_quality_info
