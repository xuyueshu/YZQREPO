...
cd `dirname $0`
source ./../config.sh
exec_dir college_students_from_other_provinces_ratio

HIVE_DB=assurance
HIVE_TABLE=college_students_from_other_provinces_ratio
...
function create_table() {
    ...
    fn_log "创建表——省外生源比例：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 省外生源比例：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
