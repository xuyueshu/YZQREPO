...
cd `dirname $0`
source ./../config.sh
exec_dir college_total_value_of_donated_equipment

HIVE_DB=assurance
HIVE_TABLE=college_total_value_of_donated_equipment
...
function create_table() {
    ...
    fn_log "创建表——捐赠设备总值 ：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
              select distinct
                      ...
                      a.donation_device_price_count as data_value,
                      'NO' as is_new,
                      FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                     app.major_donation_all_count a,
                      assurance.im_quality_data_base_info c
                      where c.data_no = '${DATA_NO}'
            "
    fn_log "导入数据 —— 捐赠设备总值 ：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                      ...
                      c.data_type as data_type,
                      a.semester_year as data_time,
                      a.donation_device_price_countl as data_value,
                      'NO' as is_new,
                      FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                      app.major_donation_all_count a,
                      assurance.im_quality_data_base_info c
                      where c.data_no = '${DATA_NO}' and a.semester_year in (select max(semester_year) from app.major_donation_all_count)
         "
}
