...
HIVE_DB=assurance
HIVE_TABLE=college_high_level_num
exec_dir college_high_level_num
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                SELECT
                  ...
                  a.all_num as data_value,
                  'NO' as is_new,
                  FROM_UNIXTIME(
                    UNIX_TIMESTAMP()
                  ) AS create_time
                FROM
                 app.teacher_high_level_count a,assurance.im_quality_data_base_info c
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                FROM
                 app.teacher_high_level_count a,assurance.im_quality_data_base_info c
                where c.data_no = '${DATA_NO}' and a.semester_year in (select max(semester_year) from teacher_high_level_count)
