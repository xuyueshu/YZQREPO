...
cd `dirname $0`
source ./../config.sh
exec_dir college_total_number_of_graduates

HIVE_DB=assurance
HIVE_TABLE=college_total_number_of_graduates
...
function create_table() {
    ...
    fn_log "创建表——毕业生总数：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
               select
                ...
                a.stime as data_time,
                a.tc as data_value,
                 'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
               (select count(code) as tc,substring(graduation_date,0,4) as stime from  model.basic_student_info
               where substring(graduation_date,0,4) in ('${NDate}','${YDate}','${ZDate}') and status = 2
               group by graduation_date) a,
               assurance.im_quality_data_base_info c
                where c.data_no = '${DATA_NO}'
            "
    fn_log "导入数据 —— 毕业生总数：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                      ...
                      FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                       (select count(s.code) as tc,substring(s.graduation_date,0,4) as stime from  model.basic_student_info s
               where s.status = 2 and substring(s.graduation_date,0,4) in (select max(substring(graduation_date,0,4)) from model.basic_student_info
                where status = 2
               )
