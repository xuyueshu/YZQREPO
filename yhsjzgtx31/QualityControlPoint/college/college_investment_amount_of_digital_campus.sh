...
cd `dirname $0`
source ./../config.sh
exec_dir college_investment_amount_of_digital_campus

HIVE_DB=assurance
HIVE_TABLE=college_investment_amount_of_digital_campus
TARGET_TABLE=im_quality_data_info
DATA_NAME=数字校园硬件建设项目投资金额
...
function create_table() {
    ...
    ) COMMENT '学校数字校园硬件建设项目投资金额表'
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——学校数字校园硬件建设项目投资金额表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            "
    fn_log "导入数据 —— 学校数字校园硬件建设项目投资金额表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            ...
            from
                 model.college_digital_campus a,
                  assurance.im_quality_data_base_info c
                where
                  c.data_name = '${DATA_NAME}' and a.semester_year in (select max(semester_year) from model.college_digital_campus )
         "
}
