...
cd `dirname $0`
source ./../config.sh
exec_dir college_developing_the_proportion_of_courses_jointly

HIVE_DB=assurance
HIVE_TABLE=college_developing_the_proportion_of_courses_jointly
TARGET_TABLE=im_quality_data_info
DATA_NO=XY_XQHZGTKFKCMSZKSKCZMSBL
...
function create_table() {
    ...
    ) COMMENT '校企合作共同开发课程门数占开设课程总门数比例 '
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——校企合作共同开发课程门数占开设课程总门数比例 ：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
              select
                      ...
                      concat(cast(count(case when is_corporate_development = 1 then course_code end)/
                      count(course_code)*100 as decimal(9,2)),'','%')
                      as data_value,
                      'NO' as is_new,
            ...
            "
    fn_log "导入数据 —— 校企合作共同开发课程门数占开设课程总门数比例 ：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
