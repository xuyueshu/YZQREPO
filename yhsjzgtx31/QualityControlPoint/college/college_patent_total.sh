...
cd `dirname $0`
source ./../config.sh
exec_dir college_patent_total

HIVE_DB=assurance
HIVE_TABLE=college_patent_total
...
function create_table() {
    ...
    fn_log "创建表——学校专利总数表：${HIVE_DB}.${HIVE_TABLE}"
}
         ...
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
            select
               c.data_no as data_no,
                ...
                a.patent_num as data_value,
                 'NO' as is_new,
                 FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from app.college_scientific_count a,
                      assurance.im_quality_data_base_info c
                      where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校专利总数表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
