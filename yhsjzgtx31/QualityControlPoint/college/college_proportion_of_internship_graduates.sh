...
cd `dirname $0`
source ./../config.sh
exec_dir college_proportion_of_internship_graduates

HIVE_DB=assurance
HIVE_TABLE=college_proportion_of_internship_graduates
TARGET_TABLE=im_quality_data_info
DATA_NO=XY_QYLYDGSXBYSBL
...
function create_table() {
    ...
    ) COMMENT '企业录用顶岗实习毕业生比例'
    LOCATION '${BASE_HIVE_DIR}/${HIVE_DB}/${HIVE_TABLE}'"

    fn_log "创建表——企业录用顶岗实习毕业生比例：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
              select
                      ...
                      nvl(concat(cast(a.num/b.num *100 as decimal(9,2)),'','%'),0) as data_value,
                      'NO' as is_new,
                      FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
                    from
                     (
                        select
                        sum(a.student_count) as num,
                        a.semester_year
                        from
                    ...
                    (
                         ...
                         count(1) as num,
                         b.semester_year
                         from model.student_graduate_employment_record a
                         left join
            ...
            "
    fn_log "导入数据 —— 企业录用顶岗实习毕业生比例：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
