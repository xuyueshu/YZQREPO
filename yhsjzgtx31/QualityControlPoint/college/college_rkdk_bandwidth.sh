...
cd `dirname $0`
source ./../config.sh
exec_dir college_rkdk_bandwidth

HIVE_DB=assurance
HIVE_TABLE=college_rkdk_bandwidth
...
function create_table() {
    ...
    fn_log "创建表——学校入口带宽表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
             select
                ...
                a.broadband_access as data_value,
                'NO' as is_new,
            ...
            "
    fn_log "导入数据 —— 学校入口带宽表：${HIVE_DB}.${HIVE_TABLE}"
}
