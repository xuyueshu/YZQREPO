...
cd `dirname $0`
source ./../config.sh
exec_dir college_teaching_material_and_monograph

HIVE_DB=assurance
HIVE_TABLE=college_teaching_material_and_monograph
...
function create_table() {
    ...
    fn_log "创建表——学校教材与专著数量表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         ...
         select distinct
            ...
            from
                (select
                cast(sum(t.textbook_count) + sum(monograph_num) as int) as t_count,t.semester_year from
                app.major_chief_editor_textbook t,
                app.college_scientific_count b
                where t.semester_year = b.semester_year
                 group by t.semester_year
                ) a,assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校教材与专著数量表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                      (select
                      cast(sum(t.textbook_count) + sum(monograph_num) as int) as t_count,
                      t.semester_year from
                        app.major_chief_editor_textbook t,
                        app.college_scientific_count b
                        where t.semester_year = b.semester_year
                         group by t.semester_year order by t.semester_year desc limit 1
                      ) a,
