...
cd `dirname $0`
source ./../config.sh
exec_dir college_training_abroad

HIVE_DB=assurance
HIVE_TABLE=college_training_abroad
...
function create_table() {
    ...
    fn_log "创建表——学校出国培训人数数量表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
          select
                ...
                a.train_nunber as data_value,
                 'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from (select count(1) as train_nunber,semester_year from  model.teacher_growing_info where level = 1
             group by semester_year) a,assurance.im_quality_data_base_info c
                where c.data_no = '${DATA_NO}'
            "
    fn_log "导入数据 —— 学校出国培训人数量表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                      (
                       select count(1) as train_nunber,semester_year from  model.teacher_growing_info t where level = 1
             group by semester_year
                        order by t.semester_year desc limit 1
                      ) a,
