...
cd `dirname $0`
source ./../config.sh
exec_dir college_quantity_of_business_system_construction

HIVE_DB=assurance
HIVE_TABLE=college_quantity_of_business_system_construction
...
function create_table() {
    ...
    fn_log "创建表——学校业务系统建设数量表：${HIVE_DB}.${HIVE_TABLE}"
}
    ...
    hive -e "
         ...
         select
                ...
                c.data_type as data_type,
                 ...
                 a.business_system_construction_num as data_value,
                  'NO' as is_new,
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                 model.college_digital_campus a,
                  assurance.im_quality_data_base_info c
                where
                  c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校业务系统建设数量表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                ...
                FROM_UNIXTIME(UNIX_TIMESTAMP()) AS create_time
            from
                 (select sum(a.overseas_student_count) as count_p,a.semester_year from model.major_enroll_student a
