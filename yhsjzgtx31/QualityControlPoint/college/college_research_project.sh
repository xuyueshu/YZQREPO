...
cd `dirname $0`
source ./../config.sh
exec_dir college_research_project

HIVE_DB=assurance
HIVE_TABLE=college_research_project
...
function create_table() {
    ...
    fn_log "创建表——学校科研项目总数表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table() {
    hive -e "
         ...
         select
            ...
            from
                (select count(1) as count_p,a.semester_year from  app.major_scientific_info a
                group  by a.semester_year
                )a,
                assurance.im_quality_data_base_info c
                where c.data_name = '${DATA_NAME}'
            "
    fn_log "导入数据 —— 学校科研项目总数表：${HIVE_DB}.${HIVE_TABLE}"
}
#插入新数据
    ...
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                      (
            select count(1) as count_p,a.semester_year from  app.major_scientific_info a
            where a.semester_year in (select max(semester_year) from app.major_scientific_info)
            group by a.semester_year
                      ) a,
                      assurance.im_quality_data_base_info c
                      where c.data_name = '${DATA_NAME}'
...
function export_table() {
    ...
    --columns 'data_no,data_name,data_cycle,data_type,data_time,data_value,is_new,create_time'
     ...
     clear_mysql_data "update assurance.im_quality_data_info set is_new = 'NO' where data_no = '${DATA_NAME}';"
    clear_mysql_data "update assurance.im_quality_data_info set is_new = 'YES' where data_no = '${DATA_NAME}' order by substr(data_time,1,4) desc limit 1;"
