...
cd `dirname $0`
source ./../config.sh
exec_dir college_average_living_quarters_area

HIVE_DB=assurance
HIVE_TABLE=college_average_living_quarters_area
...
function create_table() {
    ...
    fn_log "创建表——学校生均宿舍面积表：${HIVE_DB}.${HIVE_TABLE}"
}
...
function import_table_new() {
    hive -e "
         INSERT INTO TABLE ${HIVE_DB}.${HIVE_TABLE}
                    ...
                    from
                      (
                       select dormitory_acreage_avg,semester_year from app.college_assets_student_avg t
